...
public sealed class IntegrationSliceTraitArchitectureTests
{
    ...
    private static readonly SortedDictionary<string, string> ClassesOutsideTheSliceFamily =
        new(StringComparer.Ordinal)
        {
            ...
            ["Batch7AnchorColumnReadLedgerTests"] = "batch 7 pre-refactor control-server#208: the ledger of every place the advance still reads an anchor column off the journey row, with why each one is still there; a source-level guard that goes red when the list drifts, so hanging it off a slice would defer the guard with the slice",
