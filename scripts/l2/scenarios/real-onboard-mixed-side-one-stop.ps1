...
- 车还没到 11 号站时追加乙（`N1-3`，前侧）与丙（`N2-5`，后侧），两者的取货都在 12 号站 `N1-3_N2-5`：乙新开 12 号站的停靠，
  丙并进乙那一个——12 号站此刻不是当前下一站，可以并。三条的卸货都并进关卡那一个停靠。

**服务端怎样逐条做**（MultiStopRigCommon.ps1 头注释）：12 号站到站发一条录入请求、列出乙丙两个子批，操作员扫哪一条就装哪一条，
装完一条才发下一版清单与录入请求。所以跨需求的装货先后就是扫码先后——场景先扫乙（前侧）。卸货一次只下一条，按归属加入的先后
（甲、乙、丙）。**这意味着「先前后后」今天是加入顺序与扫码顺序给出来的，服务端没有按侧排序**；后侧需求先追加时卸货会先开后侧。
是否是规格第 20 节的缺口记在 docs/defects/20260922-cross-demand-side-order-follows-arrival.md，本场景按今天的加入顺序写。
...
- 装货各开各组、先前后后、第二条命令在第一条闭环之后才开锁：服务端库里丙的装货命令建在乙的装货提交之后；模拟器每 100 ms 采样里，
  丙那一仓第一次未锁闭晚于乙那一仓最后一次未锁闭（04）；乙的目标仓在前侧组、丙的在后侧组（05）。
...
try {
    ...
    $assertions.Add(
        'L2-MSO-04',
        '装货先前后后、第二条命令在第一条闭环之后才开锁：丙（后侧）的装货命令建在乙（前侧）的装货提交之后；模拟器采样里丙那一仓第一次未锁闭晚于乙那一仓最后一次未锁闭（REQ-0357）',
