...
一台合成车先后走两单，两段都在装货命令已下发、本站期限已过之后：

  A. 确定失败（防御路径）：对端在期限之后报 FAILED——首仓 OPERATOR_TIMEOUT、其余 NOT_STARTED，全部
     EMPTY／LOCKED／RESET。服务端判 Failed、不进恢复，自己把需求以 CANCELLED_BY_STATION_TIMEOUT 终结，结算悬空的
     装货命令，释放租约与车辆占用；再发一单，同一台车接走——这就是它被释放的证据。
     **这是防御路径：v2 车载端不产出 FAILED／OPERATOR_TIMEOUT**（期限后仓门已闭而货没动，它一直重开，
     program#55）。服务端保留结算，是因为协议允许别的车载端版本这样报；这一段只能由合成对端走到。
  B. 仓门未闭：第二单期限过了、没有结果、会话安全证据报 LOCK_NOT_CLOSED。服务端挂 STATION_TIMEOUT_DOOR_NOT_CLOSED，
     stage 留在 AwaitingLoadResult、不转 Blocked、不结束本站，挂着期间开始时间不动；门关上告警撤销；再报 COMPLETED
     按真实结果提交。门开着时会话仍是 Ready，靠的是「本车自己的装货在途」这一条豁免（按车过滤之后仍成立）。

门先关、结果后到，是真车载端的顺序，也是唯一走得通的顺序：结果一提交，本车就没有在途装货了，还开着的门不再有豁免，
会话随之离开 Ready。

control-server#121 合入之前这里用过两台车：合成对端当时按固定键缓存录入应答，同一台车扫第二单会把第一单的录入
原样重发。#121 改成按请求缓存之后，一台车先后两单能各自扫码，回到单车。
...
$peer = $Context.Onboard

# --- A1. 第一单：装货命令下发，结果不来 -------------------------------------------------------------

$null = $peer.Command('Put', 'policy', @{ loadResult = 'Manual' })
Publish-Demand $demands[0]
$failing = Wait-L2Condition -Description 'the first demand was accepted and dispatched to the pickup station' `
    -Journal $journal -Criterion 'a-stage' -TimeoutSeconds 90 `
    -Probe { Get-Runtime $demands[0].Id } -Until { param($v) [string]$v.Stage -eq 'AwaitingPickupArrival' }
Invoke-PickupLeg $failing 'a-to-pickup-intent'
$a = Wait-LoadOutstanding $failing $peer 'a-load'
...
$settled = Wait-L2Change -Description 'the determinate load failure ended demand A' `
    ...
    -Action {
        ...
        $null = $peer.Command('Put', "determinate-load-failures/$($a.Key)", @{ reasonCode = 'OPERATOR_TIMEOUT' })
    } `
...
$next = Wait-L2Change -Description 'the vehicle released by the determinate failure took the next demand' `
    -Journal $journal -Criterion 'b-accepted' -TimeoutSeconds 90 `
    -Baseline { Get-Count "SELECT COUNT(*) AS Total FROM JourneyRuntimes WHERE DemandId = '$($demands[1].Id)'" } `
    -Action { Publish-Demand $demands[1] } `
    -Probe { Get-Runtime $demands[1].Id } `
    -Until { param($before, $now) $before -eq 0 -and $now -and [string]$now.Stage -eq 'AwaitingPickupArrival' }
$doorOpen = $next.Value
$assertions.Add(
    'L2-LD-06', '确定失败结算之后，同一台车接了下一单',
    ($next.Baseline -eq 0 -and [string]$doorOpen.Stage -eq 'AwaitingPickupArrival' -and [string]$doorOpen.AgvId -eq [string]$failing.AgvId),
    "0 → AwaitingPickupArrival on $($failing.AgvId)", "$($next.Baseline) → $($doorOpen.Stage) on $($doorOpen.AgvId)")

# --- B1. 第二单：装货命令下发，操作员开着门走开 ----------------------------------------------------

Invoke-PickupLeg $doorOpen 'b-to-pickup-intent'
$b = Wait-LoadOutstanding $doorOpen $peer 'b-load'
...
$opened = Wait-L2Change -Description 'the server recorded the open door from the second peer' `
    ...
    -Action {
        $journal.Note('Peer reports a slot door not closed while the second load is still underway.')
        $null = $peer.Command('Put', 'safety', @{
            departureSafe        = $false
...
$assertions.Add(
    'L2-LD-07', '门开着时会话仍是 Ready：开着的门由本车自己在途的装货解释',
...
$assertions.Add(
    'L2-LD-10', '告警挂着期间运行时又跑了几轮：仍在 AwaitingLoadResult、同一个码、开始时间不变',
...
$assertions.Add(
    'L2-LD-11', '门关上后告警撤销、开始时间一并清掉，本站仍在等结果（关门本身不结束本站）',
...
$committed = Wait-L2Change -Description 'the completed load result settled the stop as reported' `
    ...
    -Action {
        $journal.Note("Peer reports the load COMPLETED ($($b.Key)).")
        $null = $peer.Command('Put', "answer/$($b.Key)", @{ completed = $true })
    } `
...
$assertions.Add(
    'L2-LD-12', '报 COMPLETED 后仓位操作 Committed、旅程离开 AwaitingLoadResult，告警码不在',
