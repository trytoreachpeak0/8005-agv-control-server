#Requires -Version 7
# control-server#222 fault injection, temporary: the session answer is not dropped, so the vehicle submits RESUME_AFTER_REPAIR before the restart.
...
$journal.Note('FAULT INJECTION (cs#222): the proxy is NOT armed; the answer opening the session goes through.')
