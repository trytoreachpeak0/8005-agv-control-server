...
$secondReason = Wait-L2Condition -Description 'the second demand is kept back' `
...
$assertions.Add(
    'L2-CC-07', '车空闲时，关卡已不在目录的 WIRE_TO_GATE 新需求不受理',
    ($null -eq $secondStage -and $secondReason -ne 'ACCEPTED' -and -not [string]::IsNullOrEmpty($secondReason)),
    'no journey / not ACCEPTED', "$(if ($secondStage) { $secondStage } else { 'no journey' }) / $secondReason")
...
$assertions.Add(
    'L2-CC-08', '同一变化跑过多轮仍只有两条暂停；目录变化记录按站点记下改名与删除，各指向自己那条暂停',
    (@($holds).Count -eq 2 -and
        @($changes | Where-Object { $_.StationRiotId -eq 230 -and $_.ChangeKind -eq 'RENAMED' }).Count -ge 1 -and
        @($changes | Where-Object { $_.StationRiotId -eq 210 -and $_.ChangeKind -eq 'REMOVED' }).Count -ge 1 -and
        @($changes | Where-Object { [string]::IsNullOrEmpty([string]$_.HoldId) }).Count -eq 0),
    '2 holds; 230 RENAMED, 210 REMOVED, each with a hold',
