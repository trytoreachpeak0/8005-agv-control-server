...
| `real-onboard-stale-stop-after-station-timeout` | **真的**＋协议故障代理 | **control-server#325（program#86 v2，上真车前）**：两趟不扫码、等站点期限收尾。第一趟不动报文，十轮之后车不再要子批、不再给「取消装货」、清单不挂这单（`L2-SST-03`～`05`；旧服务端不发收尾快照、旧车载端收到了不撤录入请求，两组对照都红在这里）。第二趟丢掉收尾的空清单，在「服务端已收尾、车上还挂着」的窗口里按两下「取消装货」——两下都 `REJECTED`/`WORKLIST_REVISION_STALE`、不落工作流、不重连（`L2-SST-06`、`07`）；再扫一次码——恰好一条 `SublotRejected`/`WORKLIST_REVISION_STALE`，车确认、提示区显示，之后撤掉录入（`L2-SST-08`、`10`）。断线清投影，所以「撤干净」只在不断线的第一趟判 | 同上 |
