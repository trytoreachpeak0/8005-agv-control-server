...
public static class ControlPlane
{
    ...
    public static void MapControlPlane(this WebApplication app)
    {
        ...
        // The endpoint the data plane bound, not the one configured: an onboard has to be pointed at a
        // port that is really listening, and port 0 means the operating system chose it.
        control.MapGet("/health", async () => Results.Json(ControlPlaneConventions.Envelope(engine, new
        {
            status = "live",
            listen = (await endpoints.Bound.ConfigureAwait(false)).ToString(),
        ...
        control.MapGet("/snapshot", async () => Results.Json(ControlPlaneConventions.Envelope(engine, new
        {
            listen = (await endpoints.Bound.ConfigureAwait(false)).ToString(),
            target = endpoints.Target.ToString(),
