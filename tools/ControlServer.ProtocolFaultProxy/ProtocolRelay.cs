    ...
    RelayConnections connections) : BackgroundService
{
    ...
    // Both ends frame a line with a bare LF and both hash the bytes of a line, so the relay must hand
    // each one on exactly: read it without its LF, write it back with one. A CR would not survive
    // that, and neither end sends one.
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        listener.Start();
        endpoints.MarkBound((IPEndPoint)listener.LocalEndpoint);
        try
        {
