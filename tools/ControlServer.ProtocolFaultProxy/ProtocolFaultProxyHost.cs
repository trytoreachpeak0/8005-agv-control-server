...
/// to the new session generation (8005-agv-control-server#30, #77 on v2). In L2 both ends run on one
/// machine over loopback, so that window never opens on its own.
///
...
///
/// Ported from ControlServer_MVP (78b69e38) for control-server#88; the one addition is that the
/// relay reports the endpoint it actually bound, so a test can ask for port 0.
/// </summary>
public static class ProtocolFaultProxyHost
{
    ...
    /// <summary>
    /// Resolved once at startup so no request can point the relay somewhere else. <see cref="Bound"/>
    /// completes with the endpoint the data plane actually listens on, which differs from
    /// <see cref="Listen"/> when port 0 asked the operating system to pick one.
    /// </summary>
    public sealed record RelayEndpoints(IPEndPoint Listen, IPEndPoint Target)
    {
        ...
        public void MarkBound(IPEndPoint endpoint) => bound.TrySetResult(endpoint);
    }
}
