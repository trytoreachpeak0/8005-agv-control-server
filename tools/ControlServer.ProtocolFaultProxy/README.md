...
挡在车载端和 ControlServer 的车载协议监听之间，逐行转发 NDJSON，**只在被要求时做三件事之一：吞掉某一类报文的
`DurableAck` 并断开那条连接；吞掉服务端发来的某一类应答、链路不断；不挑任何一行、直接断开**。别的一律原样转发。

从 MVP 线（`ControlServer_MVP` 的 `78b69e38`）移植到 v2，给 control-server#88 的四条真装置场景用。与 MVP 版本
只差一处：数据面在 `/health` 与 `/snapshot` 里报的是**实际绑定**的端点，所以 `listenPort=0` 可用（L1 测试就这么起）。

```
车载端 ──TCP──► 代理 ──TCP──► ControlServer :48405
                  │
...
**`RecoveryStateReport` 是唯一的例外**：v2 车载端（onboard-hmi#69）不补发未确认的旧报告，而是在补发其余报文、
发完两份快照之后发一份新 `messageId` 的新报告取代它（program#56 对 control-server#33 的结论）。
...
.\ControlServer.ProtocolFaultProxy.exe `
  --ProtocolFaultProxy:port=48415 `
  --ProtocolFaultProxy:listenPort=48416 `
  --ProtocolFaultProxy:target=127.0.0.1:48405
```
...
| 项 | 值 |
| --- | --- |
| 数据面 | 纯 TCP，`listenPort`（L2 用 48416；给 0 由系统挑，实际端口看 `/health` 的 `listen`），监听地址与控制面相同 |
| 控制面 | `/control/v1/health`、`/snapshot`、`/drop-durable-ack`、`/drop-message`、`/disconnect`、`/reset`、`/openapi.json` |
...
```powershell
PUT /control/v1/drop-message  { "runId": ..., "commandId": ..., "messageType": "LoadCancellationAuthorization", "count": 1 }
```

只看服务端发往车载端方向的行。被丢的那一行不转发，**连接照常开着**，下一行照转——车载端要等满自己的
`messageTimeoutMs` 才知道没有回答，这与应答在路上丢了、或服务端已经动作而车载端本地出错是同一个形状
（onboard-hmi#39）。`messageType` 写 `DurableAck` 直接拒：丢 ack 必然连带断链，那是 `drop-durable-ack` 的事。
