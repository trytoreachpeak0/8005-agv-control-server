...
string protocolRoot = Path.Combine(AppContext.BaseDirectory, ProtocolCandidateIdentity.Tag);
...
{
    return Fail(
        $"No vendored {ProtocolCandidateIdentity.Tag} next to the validator ({protocolRoot}). The pinned identity moved " +
...
{
    return Fail($"Vendored manifest hashes to {manifestSha256}, ProtocolCandidateIdentity.ManifestSha256 is {ProtocolCandidateIdentity.ManifestSha256}.");
}
...
{
    return Fail($"Vendored schemas/ hashes to {schemaBundleSha256}, ProtocolCandidateIdentity.SchemaBundleSha256 is {ProtocolCandidateIdentity.SchemaBundleSha256}.");
}
...
// A test calling ProtocolEnvelope.Serialize itself is exercising the envelope, not sending anything.
...
foreach (IGrouping<string, ObservedLine> byType in checkedLines.GroupBy(line => line.MessageType).OrderBy(group => group.Key, StringComparer.Ordinal))
{
    ...
    if (schemaPath is null)
    {
        ...
        {
            Record(line, [new Error("#", "messageType", $"'{byType.Key}' is not a message of {ProtocolCandidateIdentity.Tag}.", Quote(byType.Key))]);
        }
        continue;
    }
...
{
    string?[] issues = violation.Errors
        .Select(error => known.FirstOrDefault(entry => entry.Matches(violation.MessageType, error))?.Issue)
        .ToArray();
    violation.KnownIssue = issues.All(issue => issue is not null) ? issues[0] : null;
}
int unknownViolations = ordered.Count(violation => violation.KnownIssue is null);
...
File.WriteAllText(
    ...
    JsonSerializer.Serialize(new
    {
        ...
        linesInViolation = ordered.Where(violation => violation.KnownIssue is null).Sum(violation => violation.Count),
        linesInKnownViolation = ordered.Where(violation => violation.KnownIssue is not null).Sum(violation => violation.Count),
...
string summary = string.Create(
    ...
    $"Schema conformance ({ProtocolCandidateIdentity.Tag}): {checkedLines.Length} lines, {distinctLines} distinct, " +
    $"{checkedLines.Select(line => line.MessageType).Distinct().Count()} message types, {unknownViolations} distinct violations, {ordered.Length - unknownViolations} known; " +
...
foreach (Violation violation in ordered)
{
    string label = violation.KnownIssue is null ? "SCHEMA VIOLATION" : "KNOWN SCHEMA VIOLATION (" + violation.KnownIssue + ")";
...
// A violation already on file: printed and reported every run but not failed on, until the issue it
// names is fixed and the entry deleted. Every error of a violation must match an entry, so a new
// defect riding on the same line still fails. Array indices in Pointer are written as '*'.
internal sealed record KnownViolation(string MessageType, string Pointer, string Keyword, string Actual, string Issue)
{
