...
- **本发布候选不向车载端仓库写入任何内容**，只以精确 commit、构建命令与产物 SHA-256 的形式登记
  它：workflow 把该仓库 checkout 成一份 scratch 镜像，构建再从镜像的一次性克隆里做。这一条与
  谁维护它无关，改的是发布流程的隔离性质——**车载端的开发工作自 2026-09-04 起在本项目这一侧**，
  在 `w2g/*` 分支上进行、以 PR 交付；
- **协议内容由一名产品负责人决定**（2026-09-02 起，此前需双人事先批准），而正式发布仍需一份
  approval attestation——**自 2026-09-08 起一名产品负责人签**，此前两名。**AI 与 CI 不能批准**
  这一条从未改变，`tools/g1-validate.mjs` 实打实校验它。
