...
3. **在 RIoT 里确认这台车没有未结束的订单。**FAILED 的那张单已经结束，不用处理；如果还有别的单挂在这台车上，
   看它是不是服务端建的（`upperId` 以 `W2G-` 开头）：是的，不要取消，找值班工程师；不是的，按下面「急停锁着、
...
| `FAULT_RECOVERY_VEHICLE_ORDER_NOT_FINISHED` | RIoT 里这台车还有没结束的订单（排队、执行、暂停或 HANG） | 不是服务端建的单：核实它在我们的车上（按 RIoT `deviceKey`／`id`，见下一节）后只取消这一张；是服务端建的，找值班工程师。**清除之后车会重新接单**，RIoT 上还有活单时清除，车可能被那张单开走 |
...
| `FAULT_RECOVERY_CURRENT_ORDER_CANCELLED_IN_RIOT` | 旅程等的那张单是服务端自己的，被人在 RIoT 里取消或删除了 | 不走本入口：这种单不改派。#318 合入后服务端自动按同车同需求重建；在那之前旅程挡住、报警（#316）。见下面「要分清两种取消」 |
...
- **车上可能有货**（已经装货、正在装货，或者服务端说不清）：**货物绑定保留，需求不释放**，旅程转为阻断，
  ...
  #318 合入之后，服务端会自动为同一辆车、同一条需求重建开往卸货站的订单，把这趟送完，带 #318 的三道防护（见下面
  「要分清两种取消」）；2026-09-23 用户定，原话「清除后自动重建订单送完这趟」。#318 合入之前，旅程一直停在这个阻断码上、
  在看板上可见，找值班工程师。返回里 `disposition = HELD_FOR_PERSON`。
...
**第 2、3 步什么时候改由服务端做：**control-server#330 合入之后，服务端会自己取消外来订单，但只取消**正在执行、
而且 RIoT 显示就在我们车上**的那一张；排队中的订单在 RIoT 上看不出归哪辆车，服务端永远不碰。#330 合入之前，
这两步都由现场人员手工做，服务端自己从不取消不是它建的订单。
...
自动重建带三道防护：重建前先等一段时间；车处于急停、手动模式或故障时不重建；短时间内同一条需求被第二次取消，就停止
自动重建，挡住并报警。#318 合入之前，服务端只挡住、报警（`ORDER_ENDED_WITHOUT_ARRIVAL`，#316），旅程停在原处等它。
本入口不处理这种单（`FAULT_RECOVERY_CURRENT_ORDER_CANCELLED_IN_RIOT`）。只有**外来**订单才取消，而且只取消核实过在我们车上的那一张。
...
- **车上有货时清除之后，#318 合入前只能停着等人**；合入后自动重建送完这趟。
