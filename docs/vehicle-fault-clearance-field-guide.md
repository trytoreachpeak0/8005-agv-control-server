...
- **车上可能有货**（已经装货、正在装货，或者服务端说不清）：旅程码 `VEHICLE_FAULT_CLEARED_CARGO_ON_BOARD`，新单开往
  ...
  **有货时多一步：先要车证明货还在原仓**（`REQ-0362` 保留的修复续行前提「货物仍完整保留于原仓位并重新安全闭环」）。
  ...
  读数到了但证明不了——某个仓空了、门没锁、开锁输出没复位、读数有未知——就**不再自动重建**，旅程码
  `OWN_ORDER_REBUILD_CARGO_NOT_IN_PLACE`，错误级告警：货可能已经不在原仓，请到车前核对货物与仓门，找值班工程师。
...
`OWN_ORDER_REBUILD_ORDER_UNCONFIRMED`，服务端每一轮按同一个单号对账，不会建第二张。每一次终结、每一次被拦下、每一次重建，
...
- **重建被「短时二次出问题」挡住之后，服务端没有出口**：没有人工重建或放弃这趟的入口，旅程停在 `OWN_ORDER_REBUILD_STOPPED`。
