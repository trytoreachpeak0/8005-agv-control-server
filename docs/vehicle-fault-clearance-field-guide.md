...
**现场怎么做**：不要改数据库，不要在 RIoT 里手动解锁（会被当成意外恢复立即重触发）。找值班工程师和 RIoT 人员查清那张 HANG
的单是谁建的、要不要保留，由他们在 RIoT 里处理；处理完、车上没有未结束的订单之后，再按急停说明人工解除，再回到本入口清除。
「RIoT 里取消在途单多半是误操作、要确认后重建」（2026-09-22 用户定）说的是服务端自己的在途单；对一张不是服务端建的单能不能取消，
目前没有定论，由人判断。把 HANG 纳入故障模型、给服务端加闩锁下的续行顺序，是 #319。
