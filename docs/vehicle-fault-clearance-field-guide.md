...
- **故障监看仍然挂在旅程上**（停车证明的采样、升级急停、急停触发的确认、`REQ-0248` 重触发，都由引擎每轮推进这趟旅程时做），
  ...
  - 会话行仍是就绪、但车载端失联（6 秒没有任何入站）时也一样（#358）：旅程写 `ONBOARD_SESSION_LOST`，但在途单照样经 RIoT
    判，报 `FAILED` 就同一轮记故障、Hold、急停。失联本身仍然只告警（`REQ-0287`），Hold 与急停是对 `FAILED` 的回应。
    失联期间等着同车重建的那一趟不建新单，记录写明在等失联的车，车重新说话的那一轮才建。
  清除本身不依赖会话是否就绪。
