...
- **人在车前不用碰键盘**：探针按 1 到 8 的顺序，等车上所有仓都锁着再开下一仓；门弹开后，人放一个物体挡住光幕、
  取出、关门，探针从光幕和锁反馈的读数里跟着看，门锁回去才开下一仓，所以同一时刻最多开着一扇门。整台车只问两次：
  开始前确认车停稳、周边安全；八仓做完后问哪些仓的门没有作为「当时唯一弹开的那一扇」正确弹开（都对就答 `all`）。
- **会整车停下、不写记录的情况**：开锁前还有别的仓没锁；开锁后锁反馈没有变为打开（没有信号能说明门什么时候关上，
  继续开下一仓可能同时开着两扇门）；门在规定时间内没关上。停下时写 `raw/<别名>/aborted.json` 说明原因。
- **三个布尔值怎么来**：
...
**现场不拍照时**（产品负责人 2026-09-13 定），在 `window.json` 的 `photoPointersAbsentReason` 写明理由。W1-06 就只看
另外两项：证据目录形状完整，且每一仓记录里的 `fieldRecordReference` 都指向证据 `field-records/` 下真实存在的文件
（探针的逐仓原始记录）。`SUMMARY.md` 的照片一节会如实写「没有照片」和理由。
