...
**有结构性派车阻断的需求不进超时层**（例如分区归属表收录、地图上没有站点的 `AREA_STATION_NOT_FOUND`）：它没有任何车接得了，
是配置错误，已经有自己的结构性告警。它照样计龄，但不越过别人，也不发防饥饿告警。
...
**前提：MesIngest 与服务端用的是同一个钟。**年龄 = 服务端此刻 − MesIngest 写下的 `CreatedAt`，两台机器的钟差会原样变成年龄误差。
今天两者都跑在 factory01 上，是同一个钟。**将来要把 MesIngest 和服务端分到两台机器上，做那次变更的人负责**：要么保证两台对时、
误差远小于阈值，要么先改服务端的年龄算法。MesIngest 的钟快于服务端时（钟差为负），年龄按零算，不会提前进超时层。

**释放改派（批次7-10）不重置年龄**：改派的需求在 MesIngest 目录里还是那一条，`CreatedAt` 不变，年龄照旧从它算。

**需求从 MES 消失后再出现，是新的一代**：MesIngest 给它新的 `DemandId` 与新的 `CreatedAt`，年龄从头算，之前那一代的告警也不带过来。
只是在服务端这边离开目录又回来（同一个 `DemandId`）的，年龄与告警都保留。

**MesIngest 没给建单时刻**（目录项缺 `createdAt`，违约）：服务端日志打一条 Warning（事件 id 2162，每条需求一次），这条需求的年龄按 0 算，
同带里排在所有知道建单时刻的需求之后，也不做防饥饿升级，直到 MesIngest 补上。
...
只有因为暂时没车、被瞬时门禁挡住（例如查不到箱数、站点被占）而等过阈值的需求会告警，这才是防饥饿要抓的；
结构性阻断的需求不告警（见第二节）。
...
| 字段 | 意思 |
| --- | --- |
| `fullCycle` | 完整周期：一趟已完成的旅程从绑车到完成（车再次空闲、可接单），含空驶、运输与全部人工装卸。按旅程建立时刻落在窗里取样。**站点期限到期而结束的旅程也算在内**（它们同样以 `Completed` 结束，`PickupStopTermination`），样本里会混进这类较短的周期 |
| `waitToBind` | 建单至绑车等待：需求本地建单（MesIngest `CreatedAt`，与排序的等待年龄同一个起点）到它被绑进旅程。按绑车时刻取样 |
| `vehicleCount` | 样本期里承载被绑车需求的车辆数 |
| `taskCount` | 样本期里被绑车的需求条数 |
