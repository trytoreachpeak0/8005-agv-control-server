...
8005 今天唯一会自动发急停的情形是：**在途单被 RIoT 报成 FAILED，而服务端证不出车已经停住。**

这样发出的急停，按上面四个条件要自动解除，第三条「原因已经消除」要求故障事实被清除。而清除故障事实目前只有
一个入口——修复续行，它要求原订单确认为 HELD，**一张 FAILED 的单永远不满足。**「原订单已明确终结、相关阻断
收敛」该怎么判、由谁判（`REQ-0239`、`REQ-0253`），需求还没有定义。

所以现场要知道两件事：

- **车会一直锁着，服务端不会自己解除。**这是缺一个清除路径，不是车没停稳。
- **不要在 8005 的故障还在时去 RIoT 里手动解锁。**服务端会把它当成「原因消除前闩锁意外恢复 OK」，立即重新
  急停，并报 `EMERGENCY_LATCH_RELEASED_EXTERNALLY`（`REQ-0248`）。

这种情况目前没有成文规程，由值班工程师与产品负责人现场判断处置。缺陷单
`docs/defects/20260913-emergency-stop-reissued-every-evaluation-and-never-settled.md` 末节记着这件事。
