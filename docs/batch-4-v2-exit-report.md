...
**⑦ 现在是票面的「一台车先后两趟」**。本分支合入 `fp/v2-impl` 顶端（`7ed8ef2d`）后带入了 PR #121：假车载端改为按请求的业务标识缓存应答，
场景改回单车两趟。⑦ 因此在合并后的提交上**重新连跑三遍**，证据与 run 号见下文「场景⑦重跑」。

历史：出口首轮三连（run `35196671017`）跑的是「两台车各走一趟」的临时形状。当时合成车载端按固定键缓存批次录入应答，一台假车跑不完两趟
（第四节 R-4），#75 接受了这个形状，由 PR #121 修复。那三份证据原样保留，不再作为 ⑦ 的现行证据。
...
- 整张清单默认模式一遍约 19 分钟（run `35190879677` 作业 06:50:44～07:09:32，当时清单为 25 个场景；合入顶端后为 27 个）；即使 `consecutive-all` 不带场景过滤跑全清单三遍，按比例约 55～60 分钟，仍远低于 180 分钟。
...
24 份证据的 `batchId` 都是 `batch-4`，`controlServerCommit` 都是 `03ec8a8c`，每份有自己的 `runId` 与 stage root；作业 08:06:34～08:25:02（UTC），没有红、没有重跑。**其中 ⑦ 那三份是两车各一趟的历史形状，现行证据见下一小节。**
...
| `evidence/l2/20260917-b4-11-mixed-side-station-two-trips-red-001` | 测试替身：假车载端按固定键缓存应答，已由 PR #121 修复 | 同上 R-4（本票补写） |
...
| ⑦ 与其余七个场景证据的提交与协议身份不同 | ⑦ 在合并顶端后的提交上重跑、绑已发布身份；其余七个仍是 `03ec8a8c` 与候选身份，#90 在发布身份上全部重跑 |
