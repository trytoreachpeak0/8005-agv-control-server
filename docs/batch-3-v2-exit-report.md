...
- **整体 G3 仍是 `INCONCLUSIVE`。**两个 G3 runner 认领 `FP-IS-00`／`06`／`14`／`15`，`FP-IS-01`／`02`／`03`／`07` 本批次没有
  可测的面。票 17 那八片（`FP-IS-00`～`07`）的 G2 与需求线路 G3 同日也在新身份上重跑了，全部 PASS，见第三节。
...
### `FP-IS-00`～`07`：票 17 的重证，同日在新身份上重跑

这八片不是批次 3 的切片，但它们的证据同样绑着旧 manifest。为了让签 `protocol-v1.0.0` 时每一份现行证据都指向同一个 commit，
在签名之前统一重跑了一次。

| 门禁 | 结果 | 绑定 | 证据 |
| --- | --- | --- | --- |
| `CONTROL_SERVER_G2` 八片 | 全部 `PASS`（59／69／18／27／10／12／38／19 条） | `6369616` | `evidence/g2/20260912-fp-is-00-07-single-owner-6369616` |
| `ONBOARD_HMI_G2` 八片 | 全部 `PASS`（11／5／5／9／2／5／8／19 条，脚本内 G1 均 `PASS`） | 车载端 `ad0e507` | 车载端仓 `evidence/g2/20260912-fp-is-00-07-single-owner-ad0e507` |
| `G3` 需求线路（`FP-IS-04`／`05`） | `PASS`，20 条 | `6369616` | `evidence/g3/20260912-fp-is-04-05-demand-bearing-6369616` |
| `G3` 主 runner 与进程重启（`FP-IS-00`／`06`） | `PASS` | `6369616` | 与上面两个切片同一轮 |
...
| `protocol-v1.0.0` 正式发布 | 2026-09-12 已把单人签名规则移植到候选（`16e2567`），G1 PASS，两端身份跟上，本报告第三节全部门禁在新身份上重跑。`8005-agv-program` 里的候选生成器同日跟上（`a9d7a405`），重新生成与 `16e2567` 逐字节相同。产品负责人 2026-09-12 表示决定发布；写入批准、跑发布 G1、打 tag、发 Release 的脚本已备好，**由产品负责人本人运行** | **等产品负责人本人运行发布脚本** |
...
| 待办 | 卡在 |
| --- | --- |
| `protocol-v1.0.0` 正式发布 | 产品负责人本人运行 `Publish-ProtocolRelease.ps1`：写入批准 attestation、发布 G1、在 `16e2567` 上打注释 tag、发 Release（AI 与 CI 不能批准）。`FP-IS-00`～`07` 与两个切片的全部现行证据已在同一个 commit 上 |
| W1 现场核对（#17） | 走 v2 线（2026-09-12 定）；先要 `protocol-v1.0.0` 正式发布、出包、部署服务端与三台车。逐仓确认开／关／到位要在真车上看真信号、现场当时填记录拍照，与「IO 用模拟器」的无人值守验收方式冲突，需产品负责人定怎么做 |
| 篡改车恢复规程的证据 | 给 `run-staged-g3-restart.ps1` 加恢复阶段，进门禁要批准 |
| 批次 3 分支合入集成分支（`fp/b3-on-v2 → fp/v2-impl`、`w2g/b3-on-v2 → w2g/fp-v2-impl`） | 产品负责人 2026-09-12 同意，本报告随这两个 PR 一起合入 |
