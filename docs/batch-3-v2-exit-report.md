...
| 已批准硬件事实的极性常量与生产库对齐（[#44](https://github.com/trytoreachpeak0/8005-agv-control-server/issues/44)） | 生产库 2026-09-13 已按票据 35 写入逐信号极性（光幕有物为低），v2 线常量仍是 `ACTIVE_HIGH`。产品负责人定：登记为**生产切 v2 线之前**必须完成的待办，现在不改 v2 线代码；改的时候 FP-IS-14 在发布身份上的 G2/G3 证据要重跑 |
