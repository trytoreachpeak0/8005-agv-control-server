...
| REQ-0270 车载侧：真车产生告警 | 车载端 `OnboardAlarmBoard.Raise` 在产品代码里没有调用者，真车报的告警快照永远是空的；#28「与当前 AGV／停靠／操作相关的告警显示在本机界面」在真车上触发不了 | 未做；2026-09-12 已出候选告警条件清单，**等产品负责人勾选** |
...
| `protocol-v1.0.0` 正式发布 | 候选分支 `f6ee75d` 是在协议仓改为单人签名（`main` 的 `dff1686`）之前分出去的：G1 校验器要求两名不同的批准人、attestation schema `minItems: 2`、发布治理文档写两名产品负责人，而这三个文件都在 manifest 里。**单人签名在这个 commit 上过不了发布 G1**；把单人规则移植过去会改变 manifest 哈希，本报告第三节全部门禁证据随之作废 | 阻塞，**等产品负责人决定** |
...
| `protocol-v1.0.0` 正式发布 | 候选分支仍是两人签名规则，要不要把单人规则移植过去并重跑门禁（产品负责人决定）；之后 attestation ＋ 本人签名 |
| 车载告警来源 | 产品负责人勾选候选告警条件，之后在车载端接线并重跑 `FP-IS-15` 车载端门禁 |
