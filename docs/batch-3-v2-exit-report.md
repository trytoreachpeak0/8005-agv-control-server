...
**默认三条出口与两个切片的四道门禁，在已发布的 `protocol-v1.0.0` 上全部成立；W1 现场逐车逐仓 IO 核对没有做，所以按规格 8.3，批次 3 还没有出口。**
...
| **W1：三台车逐台逐仓 IO 核对、逐台放行（#17）** | **未做** | 现场作业，前置条件见第七节 |
...
| REQ-0259／0263：W1 现场核对与门禁逐台启用 | 代码与 FieldOps 工具就绪，现场没做。2026-09-12 定：走 v2 线 | 未做，前置条件见第七节 |
...
| 待办 | 卡在 |
| --- | --- |
| W1 现场核对（#17） | 协议已发布，还差：① 出包；② 生产从 v0.3.0 线（`ControlServer_MVP`）整体切到 v2 线的时间窗口，这是不兼容的协议升级，服务端和三台车要一起换；③ agv02／agv03 首次部署的现场值（`onboardInstanceId` 等），以及 agv02 那块被改过的网卡；④ 逐仓确认开／关／到位要在真车上看真实信号、现场当时填写记录并拍照，与「IO 用模拟器」的无人值守验收方式冲突。②③④ 需产品负责人决定 |
...
| 批次 3 分支合入集成分支 | 首轮已合入（服务端 #32、车载端 #40）；发布之后的这一轮跟进另开 PR，合并需产品负责人同意 |
