...
判据表的两处读数要按上面的线上序列读：`L2-IR-02` 实际栏里的「结果 1」是场景脚本把空数组用 `@()` 包成了一个元素（线上与
服务端收件箱都表明第 2 代次没有结果，唯一那条结果属于第 3 代次）；`L2-IR-04` 红在旅程阶段，是结果受理后 0.3 秒就去读、
旅程还没推进，结果、就绪、`Committed` 三项都已满足。`-001` 在场景脚本自身的 bug 上中止（断开接口只回一条连接时 `.Count`
