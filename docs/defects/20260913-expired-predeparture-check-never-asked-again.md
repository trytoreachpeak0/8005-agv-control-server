...
⚠️ 上面这次调试运行里，G3-03-05 的「去关卡的意图恰好一条」没有真正被核对：场景当时写成 `@(Invoke-L2Query ...)`，
外层 `@()` 把 `Invoke-L2Query` 已受保护的数组再包一层，条数恒为 1（2026-09-14 发现并修正）。同一断言里 RIoT 上的关卡单数
来自假 RIoT 快照、不受影响；意图条数以修正后的正式重跑为准。
