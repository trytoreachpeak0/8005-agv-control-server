...
| 01 | MesIngest V2、AcceptedDemandSnapshot、RIoT intent | 最终重读；Demand 与 TransportDemandKey 防重；Accepted snapshot + TO_PICKUP intent 同事务 | 建单未知保留原 upperId 并查询 | 唯一受理、唯一订单、可信取货到站 | 写 MesIngest；超时换号建单 |
