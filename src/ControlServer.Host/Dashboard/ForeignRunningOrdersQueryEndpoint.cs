    ...
    internal static IReadOnlyDictionary<string, string> Descriptions { get; } =
        new Dictionary<string, string>(StringComparer.Ordinal)
        {
            [ForeignRunningOrders.CancellingReason] =
                "我们车上正在运行一张不是本服务端建的订单：服务端已认定它是外来订单，发一次取消（只发一次），"
                + "在 RIoT 回查确认它已终结之前，这辆车不接新单、不接途中追加。通常几秒内自动消失，不需要人处理",
            [ForeignRunningOrders.StillRunningAfterCancelReason] =
                "服务端对这张外来订单发过一次取消，但它仍未终结：服务端不会再发第二次，这辆车不接新单、不接途中追加。"
                + "请到 RIoT 核实这张单（必要时在 RIoT 手动取消，或到车前处理），它终结后这一行自动消失",
