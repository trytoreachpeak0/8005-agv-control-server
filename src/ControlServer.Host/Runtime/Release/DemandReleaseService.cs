    ...
    OnboardJourneyPublisher publisher)
{
    ...
    /// </summary>
    /// <remarks>
    /// <b>本服务自己这次释放发出的取消不算停住。</b>取消发出、下一轮确认之后才释放；两轮之间引擎读到那张单 CANCELLED，写上
    /// <c>ORDER_ENDED_WITHOUT_ARRIVAL</c>，而且因为是本服务端自己取消的，不记重建（<c>JourneyRuntimeEngine.RecordOrderEndedInRiotAsync</c>）。
    /// 把它当成停住，服务端自己发起的改派就卡在半路（control-server#318 第一版，合成 L2 <c>reassign-when-vehicle-ineligible</c> 抓到）。
    /// 判「是不是自己取消的」与引擎判「不重建」用同一个事实：命令审计里有本服务端对这张单的 CANCEL。两处必须一致——
    /// 引擎认作「自己取消、不重建」的，这里认作「自己的释放、继续」，否则就是这里拒绝释放、那边也不重建，需求两头落空。
    /// </remarks>
    private async Task<bool> OrderStalledOrRebuildingAsync(
        JourneyRuntimeRow journey,
        ...
        CancellationToken cancellationToken)
    {
        if (string.Equals(journey.BlockReasonCode, JourneyRuntimeEngine.OrderEndedWithoutArrivalReason, StringComparison.Ordinal) &&
            await dbContext.RiotOrderCommandAudit.AsNoTracking()
                .AnyAsync(
                    row => row.TargetUpperId == current.UpperId && row.CommandType == RiotCommandTypeNames.CancelOrder,
                    cancellationToken)
                .ConfigureAwait(false))
        {
            return false;
