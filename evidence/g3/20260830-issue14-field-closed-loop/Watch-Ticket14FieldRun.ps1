...
# Read-only view of a run started by Start-Ticket14FieldRun.ps1. Everything comes out of a snapshot
...
"[{0}] stage={1} block={2} readiness={3}/{4} orders={5} dispatchEvents={6} operations={7}" -f `
    $state.observedAt,
    (@($state.journey | ForEach-Object Stage) -join ','),
    (@($state.journey | ForEach-Object BlockReasonCode | Where-Object { $_ }) -join ',' ),
    (@($state.session | ForEach-Object Readiness) -join ','),
    (@($state.session | ForEach-Object ReasonCode) -join ','),
