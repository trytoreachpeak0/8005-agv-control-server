...
| 项 | 值 |
| --- | --- |
| runId | `20260919T164517873Z` |
| agvId | `AGV-L2-001` |
| batchId | `unspecified` |
| controlServerCommit | `f27ac19979c0480968c2ec5fadb83bfe9a302da4` |
...
| stageRoot | `C:\Users\szy\AppData\Local\Temp\l2-20260919T164517873Z` |
...
| 此前已建的关卡单不被取消、不改单：同一个 UpperId 与 OrderId，仍是 CONFIRMED | PASS | `W2G-f98c59d0-e17e-485b-8d83-26996d06f4bb-GATE-1 / ORDER-000002 / CONFIRMED` | `W2G-f98c59d0-e17e-485b-8d83-26996d06f4bb-GATE-1 / ORDER-000002 / CONFIRMED` |
...
| 看板显示两条暂停及其来源：STAGING_TO_WIRE 目录变化／站点改名，WIRE_TO_GATE 目录变化／站点已不在目录 | PASS | `STAGING_TO_WIRE: 已暂停 目录变化 站点改名；WIRE_TO_GATE: 已暂停 目录变化 站点已不在目录` | `STAGING_TO_WIRE 在本图需求集合 230 派工待送取货 已暂停 目录变化，自 2026-09-19T16:48:25.0928572+00:00，站点改名 暂停 \| WIRE_TO_GATE 在本图需求集合 210 关卡 已暂停 目录变化，自 2026-09-19T16:49:23.0965408+00:00，站点已不在目录 暂停` |
...
| 210 以原名称放回目录、确认过几轮之后，等着的与新来的 WIRE_TO_GATE 需求都仍不受理，原因码 TASK_TYPE_HELD，那条暂停仍未解除（站点恢复不自动解暂停） | FAIL | `second: no journey / TASK_TYPE_HELD; third: no journey / TASK_TYPE_HELD; hold 3334c67c17c841969f1c1de1348b66f5 unreleased` | `second: AwaitingPickupArrival / ACCEPTED; third: no journey / (no reason); hold 3334c67c17c841969f1c1de1348b66f5` |
...
## 缺陷版本说明（control-server#201 红证据 A）

- 被测提交 `f27ac199` 是临时本地提交链，不进 PR：`bb187a42`（去掉 `BoundFixedTaskStationResolver.cs` 里 `holds.Holds(taskType)`
  那一段，固定站视图不再看暂停）之上摘入最终场景脚本 `8a0c782f`。
- 预期并实得：只有 `L2-CC-11` 红——210 以原名放回之后，第二条需求被受理（`second: AwaitingPickupArrival / ACCEPTED`）；
  `L2-CC-07` 仍绿（`TASK_TYPE_BINDING_STATION_NOT_IN_CATALOG`），说明站点缺席时的拒收来自目录判定，不是暂停。
