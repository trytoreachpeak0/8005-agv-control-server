...
| 项 | 值 |
| --- | --- |
| runId | `20260911T161539094Z` |
| 窗口 | `FW-FL2` |
| 现场 | L2 真装置彩排（本机） |
| 现场人员 | 无人到场，FieldOperator.psm1 run 20260911T161309202Z |
| agvId | `AGV-L2-001` |
| IO | `127.0.0.1:58412`（SIMULATOR） |
| 站点期限 | 1 分钟 |
| 旅程 | `6f6bbfe8-72e7-ae51-b41e-ead3c7e6aa0a`（T/X/NE）、`c1adda4e-a584-4456-a1bf-143495ac6dc7`（充电之后） |
| 数据库 | `local-snapshot` |
...
| FL2-T-04 | 到期才结算、没有无限拖着：结算被看到的时刻落在期限之后 0–180 秒 | PASS | `0–180 秒` | `1.1 秒` |
...
| FL2-N-01 | 窗口里的每一趟旅程都走到 Completed，派车租约都已释放 | PASS | `全部 Completed / 已释放` | `6f6bbfe8=Completed/已释放; c1adda4e=Completed/已释放` |
| FL2-N-02 | 完整闭环至少走通一次：一条需求录了 SUBLOT、装货提交、过出车前安全检查、卸货提交、需求 Succeeded（装货能提交即缺陷 20260829 现场未复现） | PASS | `至少一条需求全链路走通` | `0646c98f: SUBLOT✓ 装Committed 安检✓ 卸Committed; 1af1e2de: SUBLOT✓ 装Committed 安检✓ 卸Committed; 20c99a71: SUBLOT✓ 装Committed 安检✓ 卸Committed; 61dc47b4: SUBLOT✓ 装Committed 安检✓ 卸Committed` |
| FL2-N-03 | 每一条 Succeeded 的需求都是全链路走通的——没有哪一条跳过了 SUBLOT、安全检查或卸货提交 | PASS | `4 条都走通` | `4 条走通` |
| FL2-CH-01 | 上一趟旅程结束之后才起充电行程，触发时电量低于当时生效的触发线 | PASS | `创建于 09/12/2026 00:15:02 之后 / 触发电量 < 20` | `2026-09-11 16:15:07.1275991+00:00 / 15` |
...
| FL2-CH-05 | 释放之后接着受理下一单：下一趟旅程在充电行程结束之后创建，两趟之间只充了这一次电 | PASS | `有下一趟旅程 / 两趟之间 1 次充电` | `c1adda4e 创建于 2026-09-11 16:15:17.2242954+00:00 / 1 次` |
...
| FL2-W-01 | 现场记录由驱动脚本按实际动作写出，IO 是车上的 slots-simulator（无人到场） | PASS | `drivenBy 非空 / SIMULATOR` | `drivenBy=FieldOperator.psm1 run 20260911T161309202Z / SIMULATOR` |
...
| `drivenBy` | `FieldOperator.psm1 run 20260911T161309202Z` |
| `sublotWaitMinutes` | `1` |
| `journeyIds` | `6f6bbfe8-72e7-ae51-b41e-ead3c7e6aa0a, c1adda4e-a584-4456-a1bf-143495ac6dc7` |
| `T.demand` | `38d03949-f7a0-467f-9aa6-9b35e075f436 / L2-FW2-J1-N1-3-20260911T161309202Z / 停靠 1` |
| `X.demand` | `39db38ac-f938-45af-840f-dc853d340172 / L2-FW2-J1-N2-6-20260911T161309202Z / 停靠 2` |
| `X.faceAnswer` | `HTTP 200` |
| `NE.attempt` | `1d865f0c-7296-fe56-995f-a04700309ea4 / 仓 3 / UNLOCKING=3 WAITING_OPERATOR=3` |
