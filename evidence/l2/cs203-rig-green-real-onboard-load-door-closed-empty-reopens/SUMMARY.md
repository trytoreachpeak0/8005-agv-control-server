...
| stageRoot | `C:\Users\szy\AppData\Local\Temp\l2-20260920T180105285Z` |
...
| 期限之前空关：车载端自己重新开锁（UNLOCKING +1）、门又弹开（模拟器 OPEN、开锁输出复位）、再次等操作员 | PASS | `UNLOCKING 1→2 / WAITING_OPERATOR 增加 / OPEN/EMPTY/0/0 / 重开早于期限` | `UNLOCKING 1→2 / WAITING_OPERATOR 1→2 / OPEN/EMPTY/0/0 / 重开于期限前 37 s` |
...
| 期限之后第 2 次空关：车载端照样自己重开（UNLOCKING +1、门又弹开、再次等操作员），重开发生在期限之后 | PASS | `UNLOCKING 3→4 / WAITING_OPERATOR 增加 / OPEN/EMPTY/0/0 / 重开晚于期限` | `UNLOCKING 3→4 / WAITING_OPERATOR 3→4 / OPEN/EMPTY/0/0 / 重开于期限后 3.3 s` |
...
| 出厂配置下按取消：车载端发出针对这笔装货的取消请求，服务端授权（AUTHORIZED），范围就是这个仓 | PASS | `da649fba-5964-dc5d-873e-5ec8409336f6 / LoadCancellationAuthorization AUTHORIZED [1]` | `da649fba-5964-dc5d-873e-5ec8409336f6 / LoadCancellationAuthorization AUTHORIZED [1]` |
...
| 车辆释放：调度租约与车辆占用都释放，没有去关卡的单 | PASS | `租约已释放 / 占用已释放 / TO_GATE 0` | `ReleasedAt='2026-09-20 18:02:25.4456456+00:00' / VehicleOccupancyReleasedAt='2026-09-20 18:02:25.4456456+00:00' / TO_GATE 0` |
...
| 取消收尾之后，同一台车在 60 秒内接了下一单，并且那一单真的建出了 RIoT 单、没有被占车冲突挡住（车辆真的被释放了） | PASS | `TO_PICKUP 占用已释放 / 下一单 AwaitingPickupArrival on AGV-L2-001，TO_PICKUP 意图 CONFIRMED，没有停摆原因码` | `VehicleOccupancyReleasedAt='2026-09-20 18:02:25.4456456+00:00' / 下一单 AwaitingPickupArrival/ TO_PICKUP=CONFIRMED on AGV-L2-001` |
