...
| 项 | 值 |
| --- | --- |
| runId | `20260920T182914732Z` |
| agvId | `AGV-L2-001` |
| batchId | `unspecified` |
| controlServerCommit | `0f09a1b53d29708118d05c403187d1be7da9722f` |
| onboardHmiCommit | `2525b131d6404207e96c0c7a4fcb5c7068ea6719` |
...
| stageRoot | `C:\Users\szy\AppData\Local\Temp\l2-20260920T182914732Z` |
...
| 准入原因没有下发给车：全部 VehicleBusinessStateSnapshot 的 blockingFacts 里都没有缺绑定的原因码、需求号或任务类型；且至少有一份快照是在原因被记下之后发的（规格第 5.3 节） | PASS | `快照 ≥1 份 / 含准入原因 0 份 / 原因记下之后的快照 ≥1 份` | `快照 2 份 / 含准入原因 0 份 / 原因记下之后 2 份（原因观测于 2026-09-20T18:31:39.6231951+00:00，最后一份快照 2026-09-20T18:32:16.6241123+00:00，对照 JourneyBacklog.FirstSeenAt=2026-09-20 18:32:20.6068152+00:00）` |
...
| 只有计划、还没有清单项时，车载端不显示任何任务类型：StopDirection 已按计划腿显示方向，TaskType 为空（NEVER_INFER_UNBOUND_TASK_TYPE） | FAIL | `StopDirection 非空 / TaskType (empty)` | `StopDirection=「取货」 TaskType=「焊线→质检关卡」` |
| 车载端全程只显示被绑定的那一种任务类型：取货点与关卡两站的 TaskType 都非空，四次读数里出现过的文案只有一种，是 WIRE_TO_GATE 的（含「关卡」），不是「未知」（NEVER_INFER_UNBOUND_TASK_TYPE；不比文案全文） | PASS | `两站都显示 / 只一种文案，含「关卡」` | `en-route-to-origin: StopDirection=「取货」 TaskType=「焊线→质检关卡」；at-origin: StopDirection=「取货」 TaskType=「焊线→质检关卡」；at-destination: StopDirection=「卸货」 TaskType=「焊线→质检关卡」；completed: StopDirection=「卸货」 TaskType=「焊线→质检关卡」` |
