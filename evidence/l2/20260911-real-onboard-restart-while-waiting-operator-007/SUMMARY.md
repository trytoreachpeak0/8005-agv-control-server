...
| stageRoot | `C:\Users\szy\AppData\Local\Temp\l2-20260911T072854141Z` |
...
| 重启后车载端握手如实上报那一次未结的 attempt | PASS | `f4fbf508-422a-7150-bbbb-aa6ea79e167f` | `f4fbf508-422a-7150-bbbb-aa6ea79e167f` |
...
| 重启后按「补偿清空」开得出恢复会话，不被拒 RECOVERY_DEMAND_NOT_BLOCKED | PASS | `OPENED` | `OPENED:OPEN` |
...
| 重启前挂着的那条装货命令被补偿这一路结算掉，不会被重放进后来的会话 | PASS | `重启前挂着 / 补偿后已结算` | `重启前挂着 / 2026-09-11 07:29:19.8078082+00:00` |
| 补偿对账之后服务端会话自己回到 Ready，不必再重启客户端 | PASS | `Ready / READY` | `Ready / READY（握手上报的 pending attempts ["f4fbf508-422a-7150-bbbb-aa6ea79e167f"]）` |
| 车载端报文逐条符合 protocol-v0.3.0 的 JSON Schema（真车载端包写进 `ProtocolInbox.RequestJson` 的每一行） | PASS | `退出码 0，校验行数 > 0，未登记违约 0` | `退出码 0：Schema conformance (protocol-v0.3.0): 52 lines, 52 distinct, 14 message types, 0 distinct violations, 0 known; schema compilation 17902 ms.` |
