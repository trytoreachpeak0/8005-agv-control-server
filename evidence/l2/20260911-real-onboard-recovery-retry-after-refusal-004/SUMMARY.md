...
| 项 | 值 |
| --- | --- |
| runId | `20260911T111504171Z` |
| agvId | `AGV-L2-001` |
| batchId | `BATCH-3` |
| controlServerCommit | `c3d0b8389f57c725a8ca841816261b12447cefb8` |
...
| stageRoot | `C:\Users\szy\AppData\Local\Temp\l2-20260911T111504171Z` |
...
| 从第一次按下到收尾，车载端没有重连——服务端没有因为内容冲突掐连接 | FAIL | `SessionHello 1 条不变` | `SessionHello 1 → 2 条` |
| 单需求旅程以补偿收尾，现场收在安全状态：门关、仓空、已锁、开锁输出复位 | FAIL | `Completed / CLOSED/EMPTY/1/0` | `(not reached) / CLOSED/OCCUPIED/1/0` |
| 车载端报文逐条符合 protocol-v0.3.0 的 JSON Schema（真车载端包写进 `ProtocolInbox.RequestJson` 的每一行） | PASS | `退出码 0，校验行数 > 0，未登记违约 0` | `退出码 0：Schema conformance (protocol-v0.3.0): 29 lines, 29 distinct, 11 message types, 0 distinct violations, 0 known; schema compilation 15865 ms.` |
