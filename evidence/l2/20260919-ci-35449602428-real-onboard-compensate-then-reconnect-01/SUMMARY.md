...
| stageRoot | `C:\actions-runner\win11-01-control-server-desktop\_work\_temp\real-rig-35449602428-1\_stage\l2-20260919T144841870Z` |
...
| 补偿对账之后，恢复会话快照与补偿命令都已结清：CLOSED 那份快照被车确认，旧 revision 被取代，补偿命令被补偿结果结算 | PASS | `>= 2 recovery rows, 0 live, CLOSED snapshot acknowledged` | `5 rows, 0 live (ExceptionRecoverySessionSnapshot[8d1e7c94] state=OPEN ack=False fenced=True; ExceptionRecoverySessionSnapshot[990800fe] state=ACTION_SELECTED ack=False fenced=True; LoadCompensationCommand[374ad7c2] state= ack=True fenced=False; ExceptionRecoverySessionSnapshot[f532d4c7] state=EXECUTING ack=False fenced=True; ExceptionRecoverySessionSnapshot[0607fafe] state=CLOSED ack=True fenced=False)` |
