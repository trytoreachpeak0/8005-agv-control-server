...
| stageRoot | `C:\Users\szy\AppData\Local\Temp\l2-20260918T180135640Z` |
...
| 补偿对账之后，恢复会话快照与补偿命令都已结清：CLOSED 那份快照被车确认，旧 revision 被取代，补偿命令被补偿结果结算 | PASS | `>= 2 recovery rows, 0 live, CLOSED snapshot acknowledged` | `5 rows, 0 live (ExceptionRecoverySessionSnapshot[b6abe06b] state=OPEN ack=False fenced=True; ExceptionRecoverySessionSnapshot[75ead4c7] state=ACTION_SELECTED ack=False fenced=True; LoadCompensationCommand[f9d8adf2] state= ack=True fenced=False; ExceptionRecoverySessionSnapshot[d739fb13] state=EXECUTING ack=False fenced=True; ExceptionRecoverySessionSnapshot[d4a177ed] state=CLOSED ack=True fenced=False)` |
