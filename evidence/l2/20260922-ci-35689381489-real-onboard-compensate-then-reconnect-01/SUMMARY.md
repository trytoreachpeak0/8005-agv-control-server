...
| stageRoot | `C:\actions-runner\win11-01-control-server-desktop\_work\_temp\real-rig-35689381489-1\_stage\l2-20260922T053019772Z` |
...
| 补偿对账之后，恢复会话快照与补偿命令都已结清：CLOSED 那份快照被车确认，旧 revision 被取代，补偿命令被补偿结果结算 | PASS | `>= 2 recovery rows, 0 live, CLOSED snapshot acknowledged` | `5 rows, 0 live (ExceptionRecoverySessionSnapshot[2fb8c4c3] state=OPEN ack=False fenced=True; ExceptionRecoverySessionSnapshot[d5a00fad] state=ACTION_SELECTED ack=False fenced=True; LoadCompensationCommand[2a92b109] state= ack=True fenced=False; ExceptionRecoverySessionSnapshot[3648d096] state=EXECUTING ack=False fenced=True; ExceptionRecoverySessionSnapshot[a2a056d4] state=CLOSED ack=True fenced=False)` |
