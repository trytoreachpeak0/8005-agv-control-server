...
| 项 | 值 |
| --- | --- |
| runId | `20260920T183644948Z` |
| agvId | `AGV-L2-001` |
| batchId | `unspecified` |
| controlServerCommit | `0f09a1b53d29708118d05c403187d1be7da9722f` |
...
| stageRoot | `C:\Users\szy\AppData\Local\Temp\l2-20260920T183644948Z` |
...
| 期限之后第 1 次空关：车载端照样自己重开（UNLOCKING +1、门又弹开、再次等操作员），重开发生在期限之后 | PASS | `UNLOCKING 2→3 / WAITING_OPERATOR 增加 / OPEN/EMPTY/0/0 / 重开晚于期限` | `UNLOCKING 2→3 / WAITING_OPERATOR 2→3 / OPEN/EMPTY/0/0 / 重开于期限后 1.6 s` |
| 期限之后第 2 次空关：车载端照样自己重开（UNLOCKING +1、门又弹开、再次等操作员），重开发生在期限之后 | PASS | `UNLOCKING 3→4 / WAITING_OPERATOR 增加 / OPEN/EMPTY/0/0 / 重开晚于期限` | `UNLOCKING 3→4 / WAITING_OPERATOR 3→4 / OPEN/EMPTY/0/0 / 重开于期限后 3 s` |
...
| 出厂配置下按取消：车载端发出针对这笔装货的取消请求，服务端授权（AUTHORIZED），范围就是这个仓 | PASS | `c985aa52-ff21-205b-b47c-06177da8e9a2 / LoadCancellationAuthorization AUTHORIZED [1]` | `c985aa52-ff21-205b-b47c-06177da8e9a2 / LoadCancellationAuthorization AUTHORIZED [1]` |
...
| 车辆释放：调度租约与车辆占用都释放，没有去关卡的单 | PASS | `租约已释放 / 占用已释放 / TO_GATE 0` | `ReleasedAt='2026-09-20 18:37:52.9261499+00:00' / VehicleOccupancyReleasedAt='2026-09-20 18:37:52.9261499+00:00' / TO_GATE 0` |
...
| 取消收尾之后，同一台车在 60 秒内接了下一单，并且那一单真的建出了 RIoT 单、没有被占车冲突挡住（车辆真的被释放了） | FAIL | `TO_PICKUP 占用已释放 / 下一单 AwaitingPickupArrival on AGV-L2-001，TO_PICKUP 意图 CONFIRMED，没有停摆原因码` | `VehicleOccupancyReleasedAt='2026-09-20 18:37:52.9261499+00:00' / 下一单 Blocked/VEHICLE_OCCUPANCY_CONFLICT TO_PICKUP=CONFIRMED on AGV-L2-001` |
