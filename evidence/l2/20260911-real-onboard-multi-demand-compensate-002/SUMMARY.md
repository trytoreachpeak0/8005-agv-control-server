...
| 项 | 值 |
| --- | --- |
| runId | `20260911T082246387Z` |
| agvId | `AGV-L2-001` |
| batchId | `BATCH-3` |
| controlServerCommit | `8be28b1c01d5b839c9133fd6b0799a3d63aa0291` |
...
| stageRoot | `C:\Users\szy\AppData\Local\Temp\l2-20260911T082246387Z` |
...
| 补偿对账之后服务端会话回到 Ready（#46） | FAIL | `Ready / READY` | `RecoveryRequired / DEPARTURE_SAFETY_NOT_READY` |
...
| 关卡：三条装上车的需求逐条取空，每条卸货都提交，旅程 Completed（车载端拒收多项关卡清单时红，见 8005-agv-program#48） | FAIL | `Completed / 3 条卸货 Committed` | `AwaitingUnloadResult /  / 会话 RecoveryRequired / HANDSHAKE_INCOMPLETE / Timed out after 300s unloading journey 5a7b146c-43e2-455a-b29f-0711aa2e6331; stage AwaitingUnloadResult. / 车载端拒收关卡作业清单：PROTOCOL_SCHEMA_INVALID@not-gate ×19` |
| 收尾：停靠 1、3、4 的需求卸下成功，停靠 2 的需求是补偿判死的那一条 | FAIL | `1=Unloaded/Succeeded 2=Cancelled/Cancelled 3=Unloaded/Succeeded 4=Unloaded/Succeeded` | `1=Loaded/Accepted 2=Cancelled/Cancelled 3=Loaded/Accepted 4=Loaded/Accepted` |
| 补偿对账之后没有任何仓位操作再进 RecoveryRequired，也没有第二个恢复会话 | PASS | `RecoveryRequired 0 / 恢复会话 1` | `RecoveryRequired 0 / 恢复会话 1` |
| 车载端报文逐条符合 protocol-v0.3.0 的 JSON Schema（真车载端包写进 `ProtocolInbox.RequestJson` 的每一行） | PASS | `退出码 0，校验行数 > 0，未登记违约 0` | `退出码 0：Schema conformance (protocol-v0.3.0): 303 lines, 303 distinct, 16 message types, 0 distinct violations, 0 known; schema compilation 28440 ms.` |
