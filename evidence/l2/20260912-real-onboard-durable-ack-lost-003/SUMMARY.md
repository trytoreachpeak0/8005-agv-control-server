...
| 项 | 值 |
| --- | --- |
| runId | `20260912T130442305Z` |
| agvId | `AGV-L2-001` |
| batchId | `BATCH-3` |
| controlServerCommit | `8822a59967f31d2522751145cec16a1d16a45faf` |
...
| stageRoot | `C:\Users\szy\AppData\Local\Temp\l2-20260912T130442305Z` |
...
| 补发的 OperationResult 被服务端以 DurableAck 确认（不是掐连接） | PASS | `DurableAck for aa16e2f9-5a12-b255-afc5-f3e578b231cf on a connection after 1` | `acknowledged` |
...
| 补发之后服务端不再掐连接：丢 ack 之后没有一条连接是被服务端关掉的 | PASS | `0 connections after the drop ended by anyone but the onboard` | `0 (#1: relay dropped DurableAck for OperationResult; #2: onboard closed; #3: open)` |
...
| 车载端报文逐条符合 protocol-v0.3.0 的 JSON Schema（真车载端包写进 `ProtocolInbox.RequestJson` 的每一行） | PASS | `退出码 0，校验行数 > 0，未登记违约 0` | `退出码 0：Schema conformance (protocol-v0.3.0): 44 lines, 44 distinct, 11 message types, 0 distinct violations, 0 known; schema compilation 17178 ms.` |
