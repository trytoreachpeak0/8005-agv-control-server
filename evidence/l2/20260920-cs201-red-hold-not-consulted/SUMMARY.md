...
失败原因：The request was canceled due to the configured HttpClient.Timeout of 10 seconds elapsing.

## 身份

| 项 | 值 |
| --- | --- |
| runId | `20260919T163424253Z` |
| agvId | `AGV-L2-001` |
| batchId | `unspecified` |
| controlServerCommit | `bb187a428ba427a7f03e319b53a1c3eabf97708e` |
...
| stageRoot | `C:\Users\szy\AppData\Local\Temp\l2-20260919T163424253Z` |
...
| 此前已建的关卡单不被取消、不改单：同一个 UpperId 与 OrderId，仍是 CONFIRMED | PASS | `W2G-1732446a-da46-4c76-b4d4-8243c9c75431-GATE-1 / ORDER-000002 / CONFIRMED` | `W2G-1732446a-da46-4c76-b4d4-8243c9c75431-GATE-1 / ORDER-000002 / CONFIRMED` |
...
| 看板显示两条暂停及其来源：STAGING_TO_WIRE 目录变化／站点改名，WIRE_TO_GATE 目录变化／站点已不在目录 | PASS | `STAGING_TO_WIRE: 已暂停 目录变化 站点改名；WIRE_TO_GATE: 已暂停 目录变化 站点已不在目录` | `STAGING_TO_WIRE 在本图需求集合 230 派工待送取货 已暂停 目录变化，自 2026-09-19T16:38:38.6495628+00:00，站点改名 暂停 \| WIRE_TO_GATE 在本图需求集合 210 关卡 已暂停 目录变化，自 2026-09-19T16:39:22.0059887+00:00，站点已不在目录 暂停` |
...
## 缺陷版本说明（control-server#201 红证据，第一次）

- 被测提交 `bb187a42` 是临时本地提交，不进 PR：在 `fix/cs201-catalog-change-and-same-machine@55d64176` 之上，去掉
  `src/ControlServer.Host/Runtime/TaskTypeStations/BoundFixedTaskStationResolver.cs` 里 `holds.Holds(taskType)` 那一段——固定站视图不再看暂停。
- 结果：210 以原名放回后，排在前面的第二条 WIRE_TO_GATE 需求被受理（`AwaitingPickupArrival` / `ACCEPTED`），缺陷确实暴露；
  但当时的 `L2-CC-11` 只看第三条需求，第三条一直没有原因码，等满 60 秒后一次假 RIoT 快照请求超时（10 秒），
  运行在判据下定之前结束，所以本目录没有 `L2-CC-11` 这一行。
- 据此把 `L2-CC-11` 改成两条等待需求一起判（`8a0c782f`），重跑见 `20260920-cs201-red-hold-not-consulted-002`。
