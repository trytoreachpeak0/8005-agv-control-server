...
| `real-onboard-load-door-closed-empty-reopens-01` | FAIL（1）101s | 12 条只红 `L2-DC-08`：终结原因码为空（期望 `CANCELLED_BY_OPERATOR`）。原因是集成分支上就有的丢失更新，与本票无关，见 `../../l1/20260924-cs339-lost-update-probe/`，另开票 control-server#357 修。「装货结果未回」那段分支每一轮都走（这个场景没有断线，所以不升版），其余 11 条全过 |
