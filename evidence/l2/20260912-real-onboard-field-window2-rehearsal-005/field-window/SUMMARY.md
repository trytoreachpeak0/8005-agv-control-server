...
| 项 | 值 |
| --- | --- |
| runId | `20260912T092304950Z` |
| 窗口 | `FW-FL2` |
| 现场 | L2 真装置彩排（本机） |
| 现场人员 | 无人到场，FieldOperator.psm1 run 20260912T092021062Z |
| agvId | `AGV-L2-001` |
| IO | `127.0.0.1:58412`（SIMULATOR） |
| 站点期限 | 1 分钟 |
| 旅程 | `5a91ecae-c1e8-ca55-a57b-f84bbed7e0a8`（T/X/NE）、`d3302d72-0955-a657-b513-fc725ccd09de`（充电之后） |
| 数据库 | `local-snapshot` |
...
| FL2-T-03 | 站点期限是 1 分钟：车载端拿到的期限距驱动脚本看到服务端开始等 SUBLOT 不超过 1 分钟、且不短于它 60 秒以上 | PASS | `0–60 秒` | `59.7 秒` |
| FL2-T-04 | 到期才结算、没有无限拖着：结算被看到的时刻落在期限之后 0–180 秒 | PASS | `0–180 秒` | `2 秒` |
...
| FL2-N-01 | 窗口里的每一趟旅程都走到 Completed，派车租约都已释放 | PASS | `全部 Completed / 已释放` | `5a91ecae=Completed/已释放; d3302d72=Completed/已释放` |
| FL2-N-02 | 完整闭环至少走通一次：一条需求录了 SUBLOT、装货提交、过出车前安全检查、卸货提交、需求 Succeeded（装货能提交即缺陷 20260829 现场未复现） | PASS | `至少一条需求全链路走通` | `362e9904: SUBLOT✓ 装Committed 安检✓ 卸Committed; 6408696e: SUBLOT✓ 装Committed 安检✓ 卸Committed; 9a59498c: SUBLOT✓ 装Committed 安检✓ 卸Committed; d9146b00: SUBLOT✓ 装Committed 安检✓ 卸Committed` |
| FL2-N-03 | 每一条 Succeeded 的需求都是全链路走通的——没有哪一条跳过了 SUBLOT、安全检查或卸货提交 | PASS | `4 条都走通` | `4 条走通` |
| FL2-CH-01 | 上一趟旅程结束之后才起充电行程，触发时电量低于当时生效的触发线 | PASS | `创建于 09/12/2026 17:22:15 之后 / 触发电量 < 20` | `2026-09-12 09:22:22.7698898+00:00 / 15` |
...
| FL2-CH-05 | 释放之后接着受理下一单：下一趟旅程在充电行程结束之后创建，两趟之间只充了这一次电 | PASS | `有下一趟旅程 / 两趟之间 1 次充电` | `d3302d72 创建于 2026-09-12 09:22:36.8265239+00:00 / 1 次` |
| FL2-R-00 | 至少观测到一次车静止时的服务重启（缺陷 20260908 的发现条件：真实服务重启加一台真车） | PASS | `>= 1` | `2` |
| FL2-R1-01 | 重启后新会话（generation > 重启前的 1）回到 Ready，checkpoint r1-ready 上的会话行同样是 Ready | PASS | `generation > 1 / Ready` | `1/Ready(READY) -> 2/RecoveryRequired(HANDSHAKE_INCOMPLETE) -> 2/Ready(READY) / checkpoint: 2/Ready` |
| FL2-R1-02 | 从服务进程启动到会话 Ready 不超过 60 秒——缺陷现场是 6 分 36 秒不自愈，直到有人重启车载客户端 | PASS | `0–60 秒` | `5.6 秒` |
...
| FL2-R2-02 | 从服务进程启动到会话 Ready 不超过 60 秒——缺陷现场是 6 分 36 秒不自愈，直到有人重启车载客户端 | PASS | `0–60 秒` | `2.6 秒` |
| FL2-R2-03 | 重启时车是静止的：重启前那趟旅程已经 Completed，这正是缺陷的发现条件 | PASS | `Completed` | `Completed` |
| FL2-W-01 | 现场记录由驱动脚本按实际动作写出，IO 是车上的 slots-simulator（无人到场） | PASS | `drivenBy 非空 / SIMULATOR` | `drivenBy=FieldOperator.psm1 run 20260912T092021062Z / SIMULATOR` |
...
| `drivenBy` | `FieldOperator.psm1 run 20260912T092021062Z` |
| `sublotWaitMinutes` | `1` |
| `journeyIds` | `5a91ecae-c1e8-ca55-a57b-f84bbed7e0a8, d3302d72-0955-a657-b513-fc725ccd09de` |
| `T.demand` | `0c376047-d110-4d6b-b165-453aa858ca47 / L2-FW2-J1-N1-3-20260912T092021062Z / 停靠 1` |
| `X.demand` | `040c0211-7402-49cd-82b6-7c77c4018487 / L2-FW2-J1-N2-6-20260912T092021062Z / 停靠 2` |
| `X.faceAnswer` | `HTTP 200` |
| `NE.attempt` | `2de6755d-7794-6456-b332-51bfac5a3b27 / 仓 3 / UNLOCKING=3 WAITING_OPERATOR=3` |
| `CH.thresholds` | `出厂 20/80` |
| `CH.seen` | `AwaitingChargerArrival -> Charging -> Completed` |
| `R1.series` | `1/Ready(READY) -> 2/RecoveryRequired(HANDSHAKE_INCOMPLETE) -> 2/Ready(READY)` |
