...
| 项 | 值 |
| --- | --- |
| runId | `20260911T150731915Z` |
| agvId | `AGV-L2-001` |
| batchId | `BATCH-3` |
| controlServerCommit | `86a78b0c44117f7f999882f5f7087562d67200ae` |
...
| stageRoot | `C:\Users\szy\AppData\Local\Temp\l2-20260911T150731915Z` |
...
| 停靠 1 场景 C 接 B：门开着过期挂告警并撑满 3 分钟，回来空关后结算成 Failed，旅程自己离站 | PASS | `Failed / 离开 1/AwaitingLoadResult` | `Failed / 2/AwaitingPickupArrival（空关 2 次，重开 1 次）` |
| 停靠 2 场景 A：驱动脚本空关两轮都换来车自己重开，之后放料提交，恢复入口一次没出现 | PASS | `2 轮 / Committed / 入口未出现 / UNLOCKING >= 3` | `2 轮 / Committed / 入口未出现（看到过的按钮：LOAD_CANCELLATION） / UNLOCKING 3` |
...
| 采集器：三个物理字段都是明确的：State 不是 Unknown、锁已闭、开锁输出已复位 | PASS | `"三字段齐全且明确"` | `"slot1:State=Empty/DoorLocked=True/UnlockOutputReset=True"` |
...
| 采集器：确定失败之后旅程自己离开这一格，不需要任何人按任何按钮 | PASS | `"不是 1/AwaitingLoadResult"` | `"9/AwaitingGateArrival"` |
...
| 采集器：三个场景都有现场记录，且记录由驱动脚本按实际动作写出（模拟器 IO 下无人到场，照片不适用） | PASS | `"3 个场景 / drivenBy 非空"` | `"3 个场景 / drivenBy=FieldOperator.psm1 run 20260911T150731915Z"` |
| 采集器：窗口内恢复入口是开着的——否则决策 2 只验证了一半 | PASS | `true` | `true` |
| 采集器：每一个车停着的 checkpoint 上会话都停在 Ready——全程没有把开着的仓门当成会话故障（行驶中的帧列出不判） | PASS | `"每个车停着的 checkpoint 都是 Ready"` | `"01-00-ready=Ready（行驶中 AwaitingPickupArrival，不判）; 02-c-deadline-reached=Ready; 03-c-plus-hold=Ready; 04-b-settled=Ready（行驶中 AwaitingPickupArrival，不判）; 05-finalize=Ready（行驶中 AwaitingGateArrival，不判）"` |
| 采集器在驱动脚本写出的记录上 finalize，整窗 PASS | PASS | `exit 0 / PASS` | `exit 0 / PASS` |
| 关卡：驱动脚本把三条装上车的需求逐条取空，每条卸货都提交，旅程 Completed（多需求关卡清单被车载端拒收时红，见 8005-agv-program#48） | PASS | `Completed / 3 条卸货 Committed` | `Completed / Committed,Committed,Committed / 会话 Ready/READY` |
| 车载端报文逐条符合 protocol-v0.3.0 的 JSON Schema（真车载端包写进 `ProtocolInbox.RequestJson` 的每一行） | PASS | `退出码 0，校验行数 > 0，未登记违约 0` | `退出码 0：Schema conformance (protocol-v0.3.0): 183 lines, 183 distinct, 11 message types, 0 distinct violations, 0 known; schema compilation 19057 ms.` |
