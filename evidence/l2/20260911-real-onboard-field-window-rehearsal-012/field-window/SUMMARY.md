...
| 项 | 值 |
| --- | --- |
| runId | `20260911T151219886Z` |
| 窗口 | `FW-SC1` 装卸站收敛语义 |
| 现场 | L2 真装置彩排（本机） |
| 现场人员 | 无人到场，FieldOperator.psm1 run 20260911T150731915Z |
| agvId | `AGV-L2-001` |
...
| SC1-B-02 | 三个物理字段都是明确的：State 不是 Unknown、锁已闭、开锁输出已复位 | PASS | `三字段齐全且明确` | `slot1:State=Empty/DoorLocked=True/UnlockOutputReset=True` |
...
| SC1-B-06 | 确定失败之后旅程自己离开这一格，不需要任何人按任何按钮 | PASS | `不是 1/AwaitingLoadResult` | `9/AwaitingGateArrival` |
...
| SC1-W-01 | 三个场景都有现场记录，且记录由驱动脚本按实际动作写出（模拟器 IO 下无人到场，照片不适用） | PASS | `3 个场景 / drivenBy 非空` | `3 个场景 / drivenBy=FieldOperator.psm1 run 20260911T150731915Z` |
| SC1-W-02 | 窗口内恢复入口是开着的——否则决策 2 只验证了一半 | PASS | `True` | `True` |
| SC1-W-03 | 每一个车停着的 checkpoint 上会话都停在 Ready——全程没有把开着的仓门当成会话故障（行驶中的帧列出不判） | PASS | `每个车停着的 checkpoint 都是 Ready` | `01-00-ready=Ready（行驶中 AwaitingPickupArrival，不判）; 02-c-deadline-reached=Ready; 03-c-plus-hold=Ready; 04-b-settled=Ready（行驶中 AwaitingPickupArrival，不判）; 05-finalize=Ready（行驶中 AwaitingGateArrival，不判）` |
...
| `drivenBy` | `FieldOperator.psm1 run 20260911T150731915Z` |
| `A.slotOperationAttemptId` | `a49acae2-1cca-9d57-8bbc-cfaf7638e09c` |
| `A.phaseCounts` | `UNLOCKING=3 WAITING_OPERATOR=3` |
| `B.slotOperationAttemptId` | `c8079db4-3dc5-d854-b0a0-d0eb11eddd70` |
| `B.operationStatus` | `Failed` |
| `B.slotEvidence` | `slot1:State=Empty/DoorLocked=True/UnlockOutputReset=True` |
