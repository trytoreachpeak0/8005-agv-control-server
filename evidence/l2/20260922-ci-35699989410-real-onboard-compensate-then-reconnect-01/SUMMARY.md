...
| stageRoot | `C:\actions-runner\win11-01-control-server-desktop\_work\_temp\real-rig-35699989410-1\_stage\l2-20260922T075637606Z` |
...
| 补偿对账之后，恢复会话快照与补偿命令都已结清：CLOSED 那份快照被车确认，旧 revision 被取代，补偿命令被补偿结果结算 | PASS | `>= 2 recovery rows, 0 live, CLOSED snapshot acknowledged` | `5 rows, 0 live (ExceptionRecoverySessionSnapshot[7a5dec45] state=OPEN ack=False fenced=True; ExceptionRecoverySessionSnapshot[e141dc67] state=ACTION_SELECTED ack=False fenced=True; LoadCompensationCommand[4b259b77] state= ack=True fenced=False; ExceptionRecoverySessionSnapshot[a5d2a7cd] state=EXECUTING ack=False fenced=True; ExceptionRecoverySessionSnapshot[30a5dd3d] state=CLOSED ack=True fenced=False)` |
