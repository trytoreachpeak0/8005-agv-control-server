...
| 项 | 值 |
| --- | --- |
| 集成分支 | 本分支从 `fp/v2-impl@1e59e21b` 切出。推送前 fetch 时顶端已前移到 `a234e3ea`（cs#271 合入）：55 个文件全是新增，只在 `evidence/deploy/`（45）与 `scripts/parallel/`（10）下，与本分支改动的文件交集为空，不碰引擎、测试、L2 与假对端，所以没有合并、也没有带入顶端重跑 |
| 最终提交 | `1dc9a178` |
