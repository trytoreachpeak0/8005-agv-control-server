#Requires -Version 7
# Mutation check for the control-server#262 re-review fixes, round 5 (S1 re-review rounds 2 and 3, and the
# zero-width allowlist fix), plus the round-2 mutations with their anchors moved to the current code. Each mutation removes one guard in
...
$mutations = [ordered]@{
    ...
    'r5 installer deletes the config with Remove-Item' = @{
        File = 'Install-ParallelInstanceLocal.ps1'
        Old = '    try { Remove-ParallelInstanceDeploymentConfig -Layout $layout }'
        New = '    try { Remove-Item -LiteralPath $DeploymentConfigPath -Force -Recurse }'
    }
