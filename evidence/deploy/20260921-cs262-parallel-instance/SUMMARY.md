...
## 九、S1 聚焦复审之后（2026-09-21 下午）

独立复审结论：**S1 按上一轮的修法堵住了，没有严重项**。复审试了 47 种路径写法，没有一种能落到 MVP 那一套。
另外提了一条中等、三条疑问（第四条疑问另见下文），都已处理。本节全部输出都是在合并 `fp/v2-impl` 顶端
`913f3cfe`（cs#281）之后的树上重跑的。

推送前集成分支又前移到 `6e5d28f1`（cs#283，只改车载端替身 `FakeOnboard` 与一条架构测试）。合并后重跑了自测与
反向验证，和本节的两份输出逐行一致（比较时只屏蔽了每次随机的临时目录名），所以没有重新生成证据文件。

### M1：「服务那一步失败就中止」原来只对抛异常成立

复审用假的产品卸载脚本试了 `exit 1`、`Continue` 下的 `Write-Error`、原生命令非零退出：这几种都不中止，卸载接着
删了 8 个 V2 目录（删不到 MVP，因为逐目录复检还在）。当时安全，只是因为真实的 `Uninstall-ControlServerLocal.ps1`
恰好每种失败都抛异常——这是「今天恰好如此」，不是代码保证的。

动手前先量了各种失败方式对调用方露出什么信号（`review3-failure-surface.txt`）：

| 假产品脚本 | 抛异常 | `$?` | 退出码 | `-ErrorVariable` 收到 |
| --- | --- | --- | --- | --- |
| 只有故意静默的错误（正常卸载就是这样） | 否 | True | 0 | **2** |
| `Continue` 下 `Write-Error` | 否 | **True** | 0 | 1 |
| `exit 1` | 否 | False | 1 | 0 |
| 原生命令退出码 3 | 否 | True | 3 | 0 |

用 `-ErrorVariable` 收集错误，会把每一次正常卸载都判成失败；看 `$?` 又认不出 `Write-Error` 那种情况。所以
没有去识别更多种失败，而是反过来**要求正面确认成功**（`Invoke-ParallelProductUninstaller`）：退出码为 0；产品
脚本最后一步才写的结果文件确实是这一次新写的，内容是 `PASS`，服务名对得上；服务确实已经不在了。缺一样就抛异常。
卸载脚本只能通过这个函数调用产品脚本，自测会扫描确认这一点。

自测用 9 个假产品脚本逐一核对，每条都按报错消息核「是哪一项让它红的」。另有一个场景：结果文件事先已经存在，
这时产品脚本不会被调用（假脚本的执行标记证明它确实没被调用）。

顺带修了一个同源的小问题：删除序列没有丢弃各个动作的输出，产品脚本打印的 PASS 行会混进返回值，让返回结果从
一个对象变成两个元素的数组。以前能用，只是因为 PowerShell 会从数组里的对象上取属性。

### 疑问 2：junction 与符号链接

本机实测（`review3-junction-probe.txt`，pwsh 7.6.6）：`Remove-Item -Recurse -Force` 碰到 junction 只删链接，
不顺着删进目标。junction 在目录里面、隔两层、或者目录本身就是 junction，三种情况目标文件都还在。目录符号链接
没测：创建它要管理员权限，本机会话没有。

这是某个命令今天的行为，不是本部署的代码保证的。所以现在：要删的路径本身是链接就拒绝，目录阶段停在那里；
链接探测本身失败（例如读不到属性）也按「是链接」处理；删除序列缺了这个探测动作就拒绝启动；自测把实测行为
钉住，哪天 pwsh 改了行为自测就会红。

### 疑问 3：地图名比较太精确

`老厂前线new `（尾部空格）、`老厂前线NEW`、`MAP_25-…` 原来都会放行。现在地图名和 `MAP-25` 标识符都先规范化再比较：
Unicode NFKC、去掉格式字符、Trim、转小写、`_` 与空白视同 `-`。补了六个反例。

变异测试顺带揭出一件事（`review3-string-equality.txt`）：**PowerShell 的 `-ceq`、`-eq`、`-ccontains` 做的是按文化规则
的比较，会跳过零宽字符**，`"a<零宽空格>b" -ceq "ab"` 为 True，只有 `Ordinal` 比较能区分。我原先写的「地图名中间夹
零宽空格」那条用例，拆掉规范化之后照样是绿的，它测到的其实是 `-ceq` 自己的宽松，不是规范化。已换成「`MAP-25`
的数字中间夹零宽空格」：正则是逐字符比较的，不去掉零宽空格就匹配不上，这条才真正测到了规范化。

这种宽松放在「判定拒绝」的方向（地图名）上是安全的，因为只会多拒绝一些写法。放在「判定允许」的方向上，检查就比
文档说的弱：**车号白名单和配置键白名单都用 `-ceq`／`-ccontains`**，所以末尾多一个零宽空格的车名或键名会被当成
允许的那个。后果我分析过：这样的值写进配置后，.NET 按原样读取，车名对不上 RIoT 的任何车，键名被忽略，指不到
`agv01`。这不在本轮四条之内，已报告给调度，没有改。

### 疑问 4：安装脚本的删除

安装脚本的四处删除（替身目录、旧暂存目录、上一代包、收尾的暂存清理）原来只靠脚本开头那一次校验。现在全部改走
唯一的删目录函数 `Remove-ParallelInstanceDirectory`，每次删之前重新检查链接和两道路径检查；收尾清理补了
`-LiteralPath`。自测扫描安装和卸载两个脚本：除了安装脚本删掉它收到的那个配置文件，不允许出现别的 `Remove-Item`。

### 疑问 1：没有查

另外两个根目录（`C:\Program Files\8005 AGV`、`C:\ProgramData\8005`）下现在有哪些子目录，证据里没有。去看就是读
MVP 的安装目录，属于要先问用户的一类，所以按调度的安排，写进了第一次真装申请材料的第二步，到时一并请用户授权。

### 证据文件

| 文件 | 内容 | 结果 |
| --- | --- | --- |
| `review3-test-parallel-instance.txt` | 自测 | **134 通过，0 失败** |
| `review3-reverse-check.txt` | 在真实定义文件上做的反向验证 | **21 通过，0 失败**；结束后定义文件与运行前字节一致 |
| `review3-mutation.txt`（脚本 `review3-mutate-guards.ps1`） | 16 个变异：本轮 12 个，加上上一轮 4 个（锚点按新代码更新） | 每个都有对应用例变红，没有一个全绿；真实文件未被改动，副本已删 |
| `review3-uninstall-whatif.txt` | 在本机预演卸载 | 出厂定义被占位拒绝；填好的副本列出完整计划 |
| `review3-failure-surface.txt`、`review3-junction-probe.txt`、`review3-string-equality.txt`（各带脚本） | 本节三次实测 | 见上 |

### 这一轮自己撞上的

- 第一次跑新自测时，「普通目录被白名单拒绝」那条红了。原因是我的判据写错了：断言报错消息里「不含 junction」，
  而临时目录名 `cs262-junction-…` 里就有这个词，所以这条无论代码对错都会红。已改成正面断言拒绝理由是白名单那一条。
- 粘连扫描第一次一次传了多个文件，扫描器只报了参数错误，**根本没扫**。把这次当作没核过，改成逐个文件扫：
  只有那两处已知误报。
- `Remove-ParallelInstanceDirectory` 原来先查路径再查链接。本机自测只能在临时目录里造 junction，而临时目录一定先
  被白名单拒掉，所以链接那一支永远测不到，拆掉它也不会有测试变红。改成先查链接，两者都会拒绝，顺序只影响给出
  哪个理由。
...
| | 第一轮审查后 | 聚焦复审后 | S1 复审后 |
| --- | --- | --- | --- |
| `Test-ParallelInstance.ps1` | 54 | 105 | **134**，全绿 |
| `Invoke-ReverseCheck.ps1` | 16 | 21 | **21**，全绿 |
| 变异 | — | 6 | **16**，每个都有用例变红 |

## 十、白名单改为逐字符比较（调度 2026-09-21 定：放进本票）

第九节报告的零宽字符问题，调度决定在本票里修：白名单文档写的是「精确」，检查就该精确。

**改了哪几处。** 凡是「判定允许」方向的字符串比较，都改成逐字符比较（`Ordinal`）：

| 位置 | 原来 | 现在 | 原来会放行的 |
| --- | --- | --- | --- |
| 配置键白名单 | `-ccontains` | `Ordinal` | 已知键名末尾加一个零宽空格。.NET 会把它当成另一个键，那个看起来设了的配置其实没设 |
| 车号 `vehicleKey` | `-eq` | `Ordinal` | 末尾加零宽空格；**而且原来还不区分大小写**，全小写的 key 也能匹配上 |
| 车名 `agvId` 与 key 成对匹配 | `-cne` | `Ordinal` | 末尾加零宽空格 |
| 路径白名单「直接位于三个根之一下」 | `-ieq` | `OrdinalIgnoreCase` | 根目录名里夹一个零宽空格，也就是一个只是看起来像允许根目录的目录。路径在 Windows 上本来不区分大小写，所以这一处只把「跳过字符」改掉 |

最后一处是我向调度报告之后才找到的，当时只报了前两类。它和前两类是同一个问题，一并修了。「判定拒绝」方向的比较
（是不是 `agv01`、是不是生产路径、地图名是不是 MVP 那张）没有改：在那个方向上宽松只会多拒绝一些写法。

**自测框架里还有一处同样的问题，是新用例把它挑出来的。** 自测在每个反例前先确认「注入确实改变了定义」，比的是
注入前后定义的 JSON 文本，用的也是 `-eq`。于是只改了大小写或只加了零宽空格的反例，被框架判成「注入没有生效」
而直接报红。方向是安全的，红而不是绿，但红的理由是错的，已改成逐字符比较。

**证据。** 补了五个反例：车名、车号 key、配置键名各加零宽空格，车号 key 全小写，安装根放在伪装的根目录下。
自测 **139 项全过**（`review4-test-parallel-instance.txt`），反向验证 21 项全过（`review4-reverse-check.txt`）。
变异测试（`review4-mutation.txt`）把前三轮的 16 个变异和本节新加的 5 个一起跑，21 个都有对应用例变红。
新加的 5 个是把每处比较改回原来的写法，每个都恰好让预期的那几条变红。框架那一处改回后，四个「只改了字符」
的反例变红；配置键那条不受影响，因为它新增了一个键，JSON 的差别不止一个字符。

后果分析（调度已让复审独立核）：这几种值写进配置后，.NET 按原样读取，车名、key 对不上 RIoT 的任何车，
多出来的键被忽略，都指不到 `agv01`。所以原来的检查不危险，只是没有文档写的那么精确。
