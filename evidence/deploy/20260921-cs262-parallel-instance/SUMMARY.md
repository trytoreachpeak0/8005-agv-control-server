...
## 十一、S1 复审第三轮短审之后（2026-09-21 晚）

短审结论：**没有严重项**。复审实测了十几种产品脚本失败方式，都会中止卸载；所有情形下卸载都删不到本实例以外；配置指不到
`agv01`。它另外指出的六处都改了，零宽字符那处和第十节一起。本节的输出都是在合并 `fp/v2-impl` 顶端 `1e59e21b`（cs#257，
多停靠与途中追加）之后的树上重跑的。

| 条 | 问题 | 处理 |
| --- | --- | --- |
| 1 | 「卸载脚本只能经由函数调用产品脚本」的扫描只认 `& $productUninstaller` 和点源，复审用九种写法绕过去了 | 改成正面规则：卸载脚本里所有命令名都必须是写死的名字，不许用变量、表达式、字符串、下标、`(Get-Command …)` 或点源当命令；不许 `Invoke-Expression`、`Start-Process`、`pwsh`、`[scriptblock]::Create`、`InvokeScript`；产品脚本的文件名不许出现（查找挪进 `ParallelHost.psm1` 的 `Get-ParallelProductUninstallerPath`）。15 种绕法各写一段合成代码，扫描都要认出来；另有一段正例，证明它对唯一合法的调用不误报 |
| 2 | 「不许 `Remove-Item`」的扫描能绕过，例外带尾通配（加 `-Recurse` 也能过）；运行时删的是调用方给的任意路径 | 两个脚本里不许任何删除命令（别名、带模块名的写法、`cmd`、`robocopy`、`.Delete(`），不许动态命令名，例外只有调用产品安装和升级脚本的两处，按原文逐字列出。配置文件改由 `Remove-ParallelInstanceDeploymentConfig` 删，路径取自布局（`<运维目录>\deploy-config.json`），安装开工前由 `Test-ParallelInstanceDeploymentConfigPath` 拒绝别的路径和非普通文件。模块里的删除只允许出现在两个删除函数里。11 种绕法各写一段合成代码 |
| 3 | 零宽字符 | 见第十节；补了复审给的反例：额外键 `vehicleKey<U+200B>`，值是 `agv01` 的 key，原来会被放行并原样写进生产配置 |
| 4 | 结果文件名只精确到秒，可预测 | 文件名带 GUID，由函数自己起名。测试连调两次，读回两次传给产品脚本的路径，断言不同且带 32 位十六进制 |
| 5 | 「正面确认」有一种失败认不出：报错后继续跑、删掉服务、最后照样写 `PASS` | 今天不会发生，前提有两个：产品脚本第一句就设 `$ErrorActionPreference = 'Stop'`，而且 `PASS` 只出现一次、在最后三句之内。检查写成函数，真实产品脚本必须全过；另用四段违反前提的合成源码（改成 `Continue`、写两次 `PASS`、提前写 `PASS`、加 `trap`），每段都必须被认出来 |
| 6 | 地图规范化漏了 `map_-_25`（会变成 `map---25`）、`MAP-25.A`、各种 Unicode 横线 | 连续或混合的分隔符（空白、`_`、`-`、U+2010 到 U+2015、U+2212）折叠成一个 `-`；标识符规则改为「`map-25` 后面不是数字」。补了六个反例 |

**复审第 2 条里有一句与代码不符**：它说定义被拒时 `finally` 照样会删配置文件。实际上安装脚本的定义校验（`Install-ParallelInstanceLocal.ps1:128`）
在主 `try`（第 381 行）之前，被拒时根本进不了 `try`，配置文件不会被删，而是**会留在服务器上**。平时控制端在拷贝前已经
在本地校验过定义，服务器上再被拒的可能很小。这一点报给了调度，本轮没有改它的行为。

**有一道检查现在没有测试能触发**：`Invoke-ParallelProductUninstaller` 在调用前检查「结果文件是否已经存在」。文件名带 GUID
之后，这种情况实际上不会发生，原来那条测试换成了「文件名不可预测」。检查还留在代码里，因为它不花什么成本，而且万一发生了
会直接说明。上一轮对应的变异也从清单里去掉了，而不是留着让它「没有测试变红」。

**补了一条原来漏掉的覆盖**：安装脚本「配置文件路径不对就报错」那一行，安装脚本要管理员权限才能跑，自测没法直接运行它。
规划变异时发现，删掉这一行不会有任何测试变红。现在有一条源码断言：检查的调用和它的 `throw` 都存在，并且都在主 `try` 之前。

**这一轮踩的坑**：给正则写 `‐` 这类转义时，工具输入里的 `\u` 加四位十六进制会先被解码成字符本身，文件里写进去的是
三个肉眼几乎分不出来的横线。改了三次都「成功」了，文件却没变，最后改成在 Python 里用 `chr(92)` 拼出反斜杠才写对。字符
本身也能匹配，所以没有测试会红，只是源码没法读。

### 证据文件

| 文件 | 内容 | 结果 |
| --- | --- | --- |
| `review5-test-parallel-instance.txt` | 自测 | **187 通过，0 失败** |
| `review5-reverse-check.txt` | 真实定义文件上的反向验证 | **21 通过，0 失败** |
| `review5-mutation.txt`（脚本 `review5-mutate-guards.ps1`） | 29 个变异：第二到第四轮的 19 个（去掉了两个已失效的）加本轮 10 个 | 每个都有对应用例变红，没有锚点落空；真实文件未被改动 |

本轮 10 个变异各自红的用例：卸载脚本改用动态命令调用产品脚本，两个扫描都红（删除扫描也禁止动态命令）；卸载脚本里出现
产品脚本文件名，红一条；安装脚本用 `Remove-Item` 删配置、用 `[IO.Directory]::Delete` 删上一代，各红两条（「不给例外时
恰好两处」也会红，因为变成了三处）；删掉安装脚本的配置路径拒绝，红那条源码断言；删掉路径相等检查，红两条；删掉普通文件
检查，红一条；文件名去掉 GUID，红一条；横线不再折叠，红五条；标识符规则改回 `(-|$)`，红 `MAP-25.A` 那一条。

### 数字

| | 聚焦复审后 | S1 复审后 | 零宽字符后 | 短审后 |
| --- | --- | --- | --- | --- |
| `Test-ParallelInstance.ps1` | 105 | 134 | 139 | **187**，全绿 |
| `Invoke-ReverseCheck.ps1` | 21 | 21 | 21 | **21**，全绿 |
| 变异 | 6 | 16 | 21 | **29**，每个都有用例变红 |

## 十二、短复审之后：密钥文件的生命周期、扫描收窄为护栏（2026-09-21 夜）

短复审（`e58037a8`）结论：**没有严重项**。调度据此定了三件事：两条扫描不再追黑名单；密钥文件在每条退出路径上都要清掉；
另补几处小项。本节的输出都是在 `bd73b744` 的树上跑的，集成分支顶端 `1e59e21b` 已包含在内。

### 密钥文件 `deploy-config.json`

里面是 RIoT 调用密钥和 MesIngest 共享密钥的明文。原来只有主流程的 `finally` 会删它，而管理员检查、定义校验、配置路径校验
都在主流程之前，任何一个失败，文件就留在服务器上。现在：

| 退出路径 | 谁来删 |
| --- | --- |
| 管理员检查、读取或校验定义、生成布局、配置路径被拒、建结果目录、记录 MVP 指纹、记录已装定义、主流程中途抛出 | 安装脚本外层 `finally`（从 `Assert-Administrator` 之前开始）；配置路径被拒时不删，报残留 |
| 参数绑定失败、导入模块失败 | 控制端补查 |
| 安装进程被杀、ssh 断线 | 控制端补查（安装脚本的 `finally` 不会执行） |
| scp 成功但 ssh 起不来、scp 中途失败 | 控制端补查（从开始拷配置那一刻起生效） |
| 控制端补查时 ssh 也连不上 | 没人删，控制端打印 `SECRET_FILE_LEFT_BEHIND`，要人手工删 |

删除规则（`Remove-ParallelInstanceDeploymentConfig`）：有布局时只删布局路径；定义被拒、没有布局时只删安装脚本自己目录下的
`deploy-config.json`；必须是普通文件、不是链接。删不掉就返回原因，调用方打印 `SECRET_FILE_LEFT_BEHIND`；安装本身成功却
留下文件时改为报错。控制端补查只删一个文件：路径和运维目录在控制端由布局算出，服务器端函数只接受
`<运维目录>\deploy-config.json`。

测试：三种退出（接受后抛出、路径被拒、定义被拒）各断言删除或报残留，目录、链接、不存在各一条；两个端到端测试在临时目录里
真的运行安装脚本，让它在第一个检查退出（本机未提权，停在管理员检查），断言旁边的配置被删、别处的配置报出残留。控制端
补查的命令按脚本里的写法拼出来、经 `-EncodedCommand` 在本机模拟了三种情况（`review8-control-after-check.txt`）。

**这一段踩了一次坑，是我自己改出来的**：控制端第一版给服务器端函数传了「由被检查的那个路径本身构造出来的布局」，于是
「必须等于布局路径」这道检查变成自己和自己比，永远成立，模拟时放在别处的文件被删掉了。改回不传布局、只按「运维目录下
那一个文件」的规则，模拟结果才对。

兜不住的那一种（最后一行）后果有限：运维目录在控制端建时就收成只有 SYSTEM 和 Administrators 能读，而管理员本来就能拿到
这台机器上的一切。复审转述的「两个秘密本来就在服务注册表和机器环境变量里」核下来不全对：RIoT 密钥写进的是执行安装那个
账户的用户级环境变量，MesIngest 共享密钥要到安装后段才写进本服务注册表，早退出时还没写到任何别处。文档用的是前一种说法。

### 扫描：统一判据，收窄说法

- 模块侧原来用一套更弱的规则，`ParallelHost.psm1` 根本不扫。现在两个脚本、两个模块用同一个扫描函数、同一套规则；例外
  按「函数名::原文」逐字列出；每个文件另有一条断言：去掉例外后，找出来的恰好就是那几处（模块是 12 处，先独立数出来再对）。
- 补上 `ForEach-Object Delete`、`-MemberName Delete`、`% Delete`。
- 所有注释、README 与部署文档都改成「防回归护栏，不是证明」，列出已知绕法类别，并写明真正的保证是哪几条构造。

### 小项

- 前提检查挪进 `ParallelHost.psm1`，卸载时由 `Get-ParallelProductUninstallerPath` 对**实际要调用的那一份**再做一次（通常是已装包
  或上一代包里的，不是仓库里的），不过就中止。
- `sc.exe delete` 返回 1072：复审担心会误报，**实测不会**（`review8-sc1072-probe.txt`）。产品脚本在 `sc.exe` 之后还会跑
  `netstat.exe`，调用方拿到的退出码是后者的 0。文档写明这依赖两条命令的先后。
- 地图标识符：分隔符可省略，`MAP25` 也被拒；左边界、`MAP.25`、`MAP/25`、`MAP-025` 写明为限度。

### 证据文件

| 文件 | 结果 |
| --- | --- |
| `review8-test-parallel-instance.txt` | 自测 **209 通过，0 失败** |
| `review8-reverse-check.txt` | 反向验证 **21 通过，0 失败** |
| `review8-mutation.txt`（脚本 `review8-mutate-guards.ps1`） | **39 个变异，每个都有对应用例变红**；锚点全部恰好命中一处，没有一次自测中途崩掉；真实文件未被改动 |
| `review8-control-after-check.txt`（脚本同名 `.ps1`） | 控制端补查的三种情况 |
| `review8-sc1072-probe.txt`（脚本同名 `.ps1`） | 1072 不会传到调用方 |

变异测试在这一轮还挑出了测试自己的三个问题，都改了：有一个变异锚点命中两处（分不清是哪一处让测试红的）；配置文件的
用例彼此依赖，一个变异会让自测中途崩掉、另一个会连锁多红一条；把旧用例改写成删除函数的用例之后，路径检查函数的
「必须是普通文件」没人覆盖了。
