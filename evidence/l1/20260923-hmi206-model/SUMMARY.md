...
## 第三轮：merge 集成分支顶端 `012c31b2`（cs#330）之后（`e808c011`）

| 文件 | 结果 |
| --- | --- |
| `08-after-merging-012c31b2-model-and-architecture-tests.txt` | 三个 `ReconnectModel*` 类（含 CI 那一批 200 个固定种子）加 `*ArchitectureTests`：92 条全过，3 条只在显式要求时跑 |
